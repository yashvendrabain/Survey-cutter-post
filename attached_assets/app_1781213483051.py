...
def _render_output_structure_gate() -> bool:
    ...
    """
    app = _require_streamlit()
    # Default to one-sheet on first entry (user can switch via "Change output
    # structure"). This makes "Keep All Questions on One Sheet" the default.
    if app.session_state.get("wiz_output_structure") is None and \
            not app.session_state.get("wiz_structure_user_touched"):
    ...
    with col_b:
        ...
        if app.button("Keep All Questions on One Sheet", key="wiz_struct_one", use_container_width=True):
            app.session_state["wiz_output_structure"] = "one_sheet"
