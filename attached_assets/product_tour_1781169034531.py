...
(function(){
  ...
  if (pdoc && pdoc.head && !pdoc.getElementById("sae-tour-style")) {
    ...
    sty.textContent =
      ...
      "#sae-tour-arrow{position:fixed;z-index:2000000;pointer-events:none;font-size:34px;"+
...
(function(){
  ...
  function drawArrow(tgt){
    ...
    if (cy < vh*0.6){
      arrow.textContent = "\u2B07";
      arrow.style.left = Math.min(Math.max(8, cx-17), window.parent.innerWidth-40)+"px";
      arrow.style.top  = Math.max(8, r.top-44)+"px";
    } else {
      arrow.textContent = "\u2B06";
      arrow.style.left = Math.min(Math.max(8, cx-17), window.parent.innerWidth-40)+"px";
      arrow.style.top  = Math.min(vh-50, r.bottom+8)+"px";
    }
