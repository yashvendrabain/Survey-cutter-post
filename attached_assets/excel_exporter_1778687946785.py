...
def export_single_cuts(
    ...
    used_sheet_names = _reserved_sheet_names().union({"_RawData", "_Options"})
...
def _build_raw_data_sheet(
    ...
    _add_named_range(workbook, "respondent_id_data", "_RawData", "$A:$A")
    for index, column in enumerate(columns, start=2):
        ...
        _add_named_range(workbook, column.data_name, "_RawData", f"${col_letter}:${col_letter}")
...
def _build_options_sheet(
    ...
    worksheet.cell(row=1, column=2, value="None_options")
    worksheet.cell(row=2, column=2, value="(All)")
    _add_named_range(workbook, "None_options", "_Options", "$B$2:$B$2")

    for col_index, column in enumerate(context.columns, start=3):
...
def _live_write_theme_sheet(
    ...
    row_index, sheet_filters = _live_write_sheet_header_and_filters(
        workbook,
        worksheet,
        theme_name,
...
def _live_write_sheet_header_and_filters(
    workbook: Any,
    worksheet: Any,
    theme_name: str,
    demographic_questions: list[Any],
    ...
    worksheet.cell(row=3, column=1, value="DEMOGRAPHIC FILTERS").font = _live_font(bold=True)
    _live_fill_row(worksheet, 3, 10, "F2F2F2")
    sheet_filters: list[dict[str, str]] = []
    used_filter_names: set[str] = set()
    categories = (demo_priority or {}).get("categories", {})

    for offset, question in enumerate(demographic_questions[:8]):
        col_index = offset + 1
        ...
        ).font = _live_font(bold=True, size=9)
        cell = worksheet.cell(row=5, column=col_index, value="(All)")
        _add_named_cell(workbook, filter_name, worksheet, cell.coordinate)
        _add_dropdown_to_cell(worksheet, cell.coordinate, f"={column.header}_options")
    ...
    worksheet.cell(row=7, column=1, value="OPTIONAL CUSTOM FILTERS").font = _live_font(bold=True)
    _live_fill_row(worksheet, 7, 10, "F2F2F2")
    _live_write_custom_filter_slot(workbook, worksheet, 8, 1, "F_Custom1_Q", "F_Custom1_V")
    _live_write_custom_filter_slot(workbook, worksheet, 9, 1, "F_Custom2_Q", "F_Custom2_V")
    return 11, sheet_filters
...
def _live_write_custom_filter_slot(
    ...
    worksheet.cell(row=row_index, column=col_index, value=f"Filter {1 if '1' in question_name else 2}:")
    q_cell = worksheet.cell(row=row_index, column=col_index + 1, value="(None)")
    worksheet.cell(row=row_index, column=col_index + 2, value="=")
...
def _live_write_question_block(
    ...
    worksheet.cell(
        ...
        column=1,
        value=f"{question.canonical_id} - {question.question_text}",
    ).font = _live_font(bold=True, size=11)
    row_index += 1

    q_prefix = _safe_defined_name(question.canonical_id)
    fq_name = f"{q_prefix}_F_Q"
    fv_name = f"{q_prefix}_F_V"
    ct_name = f"{q_prefix}_CT"

    worksheet.cell(row=row_index, column=1, value="Per-question filter").font = _live_font(bold=True, size=9)
    worksheet.cell(row=row_index, column=6, value="Cross-tab by").font = _live_font(bold=True, size=9)
    row_index += 1
    worksheet.cell(row=row_index, column=1, value="Filter:")
    fq_cell = worksheet.cell(row=row_index, column=2, value="(None)")
    worksheet.cell(row=row_index, column=3, value="=")
    fv_cell = worksheet.cell(row=row_index, column=4, value="(All)")
    ct_cell = worksheet.cell(row=row_index, column=6, value="(None)")
...
def _live_write_categorical_table(
    ...
    for offset, row in enumerate(rows):
        ...
        worksheet.cell(row=excel_row, column=2, value=count_formula)
    data_end = max(data_start, data_start + len(rows) - 1)
    for row_index in range(data_start, data_end + 1):
        worksheet.cell(
            row=row_index,
            column=3,
            value=f"=IFERROR(B{row_index}/SUBTOTAL(9,B{data_start}:B{data_end}),0)",
        ).number_format = "0.0%"
        worksheet.cell(
            row=row_index,
            column=4,
            value=f"=SUBTOTAL(9,B{data_start}:B{data_end})",
        ).number_format = "#,##0"
    ...
    if result.question_type is QuestionType.NUMERIC_ALLOCATION:
        for option_id, payload in (result.per_option_stats or {}).items():
            ...
            for metric in ("Mean", "Min", "Max"):
                ...
                worksheet.cell(row=row_index, column=4, value=_build_numeric_count_formula(column.data_name, sheet_filters, fq_name, fv_name))
                row_index += 1
...
def _live_write_cross_tab_table(
    ...
    for offset in range(8):
        ...
        worksheet.cell(
            row=start_row,
            column=col_index,
            value=f'=IFERROR(INDEX(INDIRECT({ct_name} & "_options"),{offset + 2}),"")',
        )
    ...
    for row_offset, row in enumerate(rows):
        ...
        for offset in range(8):
            ...
            worksheet.cell(
                row=excel_row,
                column=col_index,
                value=f'=IF({ct_name}="(None)","",{countifs_formula})',
            )
        worksheet.cell(
            row=excel_row,
            column=first_col + 9,
            value=f"=SUM(G{excel_row}:N{excel_row})",
        )
...
def _build_countifs_formula(
    ...
    pairs = _filter_formula_pairs(sheet_filters)
    pairs.extend(_dynamic_filter_pairs("F_Custom1_Q", "F_Custom1_V"))
    pairs.extend(_dynamic_filter_pairs("F_Custom2_Q", "F_Custom2_V"))
    pairs.extend(_dynamic_filter_pairs(fq_name, fv_name))
    pairs.append((data_name, _excel_criteria(criteria)))
    pairs.extend(extra_pairs or [])
    return "=COUNTIFS(" + ",".join(item for pair in pairs for item in pair) + ")"
...
def _build_numeric_formula(
    ...
    criteria_pairs = _filter_formula_pairs(sheet_filters)
    criteria_pairs.extend(_dynamic_filter_pairs("F_Custom1_Q", "F_Custom1_V"))
    criteria_pairs.extend(_dynamic_filter_pairs("F_Custom2_Q", "F_Custom2_V"))
    criteria_pairs.extend(_dynamic_filter_pairs(fq_name, fv_name))
    criteria_pairs.append((data_name, '"<>"'))
    args = ",".join(item for pair in criteria_pairs for item in pair)
    if metric == "Mean":
        return f"=IFERROR(AVERAGEIFS({data_name},{args}),0)"
    if metric == "Min":
        return f"=IFERROR(MINIFS({data_name},{args}),0)"
    if metric == "Max":
        return f"=IFERROR(MAXIFS({data_name},{args}),0)"
    return "0"
...
def _build_numeric_count_formula(
    ...
    return _build_countifs_formula(data_name, "<>", sheet_filters, fq_name, fv_name)


def _filter_formula_pairs(sheet_filters: list[dict[str, str]]) -> list[tuple[str, str]]:
    return [
        (
            sheet_filter["data_name"],
            f'IF({sheet_filter["filter_name"]}="(All)","*",{sheet_filter["filter_name"]})',
        )
        for sheet_filter in sheet_filters
    ]


def _dynamic_filter_pairs(question_name: str, value_name: str) -> list[tuple[str, str]]:
    return [
        (
            f'INDIRECT(IF({question_name}="(None)","respondent_id_data",{question_name} & "_data"))',
            f'IF({question_name}="(None)","<>",IF({value_name}="(All)","*",{value_name}))',
        )
    ]
...
def _add_named_cell(workbook: Any, name: str, worksheet: Any, cell_ref: str) -> None:
    _add_named_range(workbook, name, worksheet.title, f"${cell_ref}")
