...
class TestRawDecoder(unittest.TestCase):
    ...
    def test_respondent_id_falls_back_to_first_column_when_absent(self) -> None:
        dataframe, report = decode_raw_data(
            str(RAW_DECODER_NO_ID_CSV_PATH), RAW_DECODER_DATA_MAP
        )

        self.assertNotIn("respondent_id", dataframe.columns)
        self.assertEqual(dataframe.columns[0], "Q3")
        self.assertNotIn(
            "no respondent ID column found; generated sequential IDs",
            report.warnings,
        )
