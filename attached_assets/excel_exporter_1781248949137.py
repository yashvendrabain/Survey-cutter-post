...
def export_single_cuts(
    ...
    with export_memory.memory_step("build_skipped_questions_sheet"):
        _live_write_skipped_questions(workbook, schema, skips, cross_cut_skips)
...
def export_winners_vs_laggards_workbook(
    ...
    for theme_name, theme_results in theme_groups:
        ...
        row_index, local_filters, custom_local_filters = _build_theme_local_filters_block(
            workbook,
            worksheet,
            1,
            theme_prefix,
...
def _all_questions_dropdown_entries(
    ...
    for filter_option in filter_question_options(schema):
        ...
        if filter_option.question_type in {
            ...
            # Build value_lookup so this question's values resolve in the custom
            # filter (previously empty -> MATCH failed -> filter did nothing).
            sgl_value_lookup: list[dict[str, str]] = []
            for value_option in filter_option.values:
                value_label = str(value_option.label)
                if not value_label:
                    continue
                sgl_value_lookup.append(
                    {
            ...
            # Defensive fallback: if no filter values were provided, derive them
            # from the question's option_map so single-selects always resolve.
            if not sgl_value_lookup:
                for opt_label in (getattr(question, "option_map", {}) or {}).values():
                    opt_label = str(opt_label)
                    if not opt_label:
                        continue
                    sgl_value_lookup.append(
                        {
            ...
            entries.append(
                {
                    ...
                    "option_values": (),
                    "value_lookup": tuple(sgl_value_lookup),
                }
            )
            continue
...
def _build_theme_local_filter_helper_column(
    ...
    context: _LiveWorkbookContext,
    custom_local_filters: list[dict[str, str]] | None = None,
) -> str:
    """Append a per-theme local-filter pass mask to _RawData.

    Includes BOTH the standard local filters AND the per-sheet custom filters
    (the custom ones were previously omitted, so they never filtered the data).
    """
    ...
    # Per-sheet CUSTOM filters: build a match column each, using the custom
    # (resolved_column / INDIRECT) formula, and fold into the same pass mask.
    for custom in (custom_local_filters or []):
        match_name = _safe_defined_name(f"{custom['match_basename']}_match_data")
        worksheet.cell(row=1, column=next_col, value=match_name.removesuffix("_data"))
        for row_index in range(2, last_row + 1):
            _live_formula(
                worksheet,
                ...
                _raw_custom_filter_match_formula(
                    row_index=row_index,
                    question_name=custom["question_name"],
                    value_name=custom["value_name"],
                    wrapped_name=custom["wrapped_name"],
                    resolved_column_name=custom["resolved_column_name"],
                    criteria_name=custom["criteria_name"],
                ),
                1,
            )
        col_letter = _openpyxl_column_letter(next_col)
...
def _humanise_skip_reason(reason: str) -> str:
    if not reason:
        return "Skipped"
    return _SKIP_REASON_LABELS.get(reason, str(reason).replace("_", " ").capitalize())


def _live_write_skipped_questions(
    workbook: Any,
    schema: SurveySchema,
    skips: list[SkipRecord],
    cross_cut_skips: list[SkipRecord],
) -> None:
    """One sheet listing every skipped question (single-cut + cross-cut) with a
    plain-language reason, so the reader can see exactly what was left out."""
    worksheet = workbook.create_sheet("Skipped_Questions")
    title = worksheet.cell(row=1, column=1, value="Questions skipped during analysis")
    title.font = _live_font(bold=True, size=13)
    total = len(skips) + len(cross_cut_skips or [])
    worksheet.cell(
        row=2, column=1,
        value=(f"{total} question(s) were skipped. They are listed below with the "
               "reason. Nothing here affects the numbers on the other sheets."),
    )
    header_row = 4
    _live_header_row(
        worksheet, header_row,
        ["#", "Source", "Question ID", "Question", "Type", "Reason", "Details"],
    )
    row_index = header_row + 1
    counter = 0

    def _question_text(canonical_id: str) -> str:
        try:
            q = schema.get_question(canonical_id)
        except Exception:
            q = None
        return getattr(q, "question_text", "") or ""

    def _write_skip(skip: SkipRecord, source: str) -> None:
        nonlocal row_index, counter
        counter += 1
        try:
            qtype = skip.question_type.value
        except Exception:
            qtype = str(getattr(skip, "question_type", "") or "")
        values = [
            counter,
            source,
            getattr(skip, "canonical_id", "") or getattr(skip, "question_id", ""),
            _question_text(getattr(skip, "canonical_id", "")),
            qtype,
            _humanise_skip_reason(getattr(skip, "skip_reason", "")),
            getattr(skip, "details", "") or "",
        ]
        for col_index, value in enumerate(values, start=1):
            worksheet.cell(row=row_index, column=col_index, value=value)
        row_index += 1
    ...
    for skip in skips:
        _write_skip(skip, "Single cut")
    for skip in (cross_cut_skips or []):
        _write_skip(skip, "Cross cut")

    if counter == 0:
        worksheet.cell(
            row=row_index, column=1,
            value="No questions were skipped \u2014 every question was analysed.",
        )
...
def _build_theme_local_filters_block(
    ...
    custom_local_filters: list[dict[str, str]] = []
    for slot in range(1, context.workbook_custom_filter_count + 1):
        row_index, _custom_meta = _write_theme_custom_filter(
            workbook,
            worksheet,
            row_index,
            theme_prefix,
        ...
        custom_local_filters.append(_custom_meta)
    return row_index + 2, local_filters, custom_local_filters
...
def _write_theme_custom_filter(
    ...
    # Return metadata so the caller can build a per-sheet custom MATCH column
    # (previously omitted -> per-sheet custom filters never filtered the data).
    custom_meta = {
        "is_custom": True,
        "question_name": resolved_q,
        "value_name": resolved_v,
        "wrapped_name": wrapped_name,
        "resolved_column_name": resolved_column,
        "criteria_name": f"{theme_prefix}_{global_question_name[:-2]}_resolved_criteria",
        "match_basename": f"{theme_prefix}_{global_question_name[:-2]}_local",
    }
    return row_index + 1, custom_meta
...
def _live_write_rank_order_table(
    ...
    # Rank column (item 5): rank options by Weighted Average, highest = 1.
    # Live RANK over the WA column so it re-ranks when filters change.
    rank_col = last_col + 1
    worksheet.cell(row=header_row, column=rank_col, value="Rank").font = _live_font(bold=True)
    worksheet.cell(row=subheader_row, column=rank_col, value="by avg").font = _live_font(bold=True)
    for row in (header_row, subheader_row):
        worksheet.cell(row=row, column=rank_col).fill = _live_fill("F2F2F2")
    if result.rows:
        wa_sorted = sorted(
            range(len(result.rows)),
            key=lambda i: -_rank_weighted_average_from_counts(
                result.rows[i].counts_per_rank, rank_points, result.total_respondents
            ),
        )
        cached_rank = {idx: pos + 1 for pos, idx in enumerate(wa_sorted)}
        for offset in range(len(result.rows)):
            excel_row = data_start + offset
            _live_formula(
                worksheet,
                excel_row,
                rank_col,
                f"=RANK(C{excel_row},$C${data_start}:$C${data_end})",
                cached_rank.get(offset, offset + 1),
            ).number_format = "0"
    last_col = rank_col
...
def _live_write_categorical_table(
    ...
    if rows and is_multi_select and live_respondent_denominator_formula:
        # Multi-select: respondents who answered any sub-column, filter-aware.
        total_respondents_formula = "=" + live_respondent_denominator_formula
    elif rows:
        # Single-select: the visible (filtered) counts sum to the answering
        # respondents, so SUBTOTAL over the Count column tracks the filter.
        total_respondents_formula = f"=SUBTOTAL(9,B{data_start}:B{data_end})"
    else:
        total_respondents_formula = None
    _write_total_respondents_row(
        worksheet, total_row, respondent_total, 3, total_respondents_formula
    )
    table_end_row = total_row
    if total_responses is not None:
        table_end_row = total_row + 1
        _write_total_responses_row(worksheet, table_end_row, total_responses, 3)
    _openpyxl_add_table(worksheet, start_row, 1, table_end_row, 3)
...
def _live_write_cross_tab_table(
    ...
    # Column-wise totals: for EACH cross-cut group column, count respondents in
    # that group (passing filters). Previously only the rightmost total column
    # had a value, leaving the per-group totals blank.
    for offset in range(max_groups):
        group_col = first_col + 1 + offset
        header_ref = f"{_openpyxl_column_letter(group_col)}${header_row}"
        col_total_formula = _build_countifs_formula(
            "respondent_id_data",
            "<>",
            sheet_filters,
            fq_name,
        ...
        _live_formula(
            ...
            total_row,
            group_col,
            f'=IF(OR({ct_name}="(None)",{header_ref}=""),"",{col_total_formula})',
            0,
        )
    _live_formula(
        worksheet,
...
def _live_distribution_rows(
    ...
    if isinstance(result, SingleSelectResult):
        ...
        for sub_column_id, payload in result.selections.items():
            ...
            rows.append(
                {
                    "label": _resolve_ms_option_label(
                        question, sub_column_id, payload, column
                    ),
                    "data_name": column.data_name,
                    "criteria": "Selected",
                    "metric_kind": "count",
                    ...
                    "cached_value": int(payload.get("count", 0) or 0),
                    "sort_count": int(payload["count"]),
                    "denominator_data_names": denominator_data_names,
                }
            )
...
def _label_is_uninformative(label: str, sub_column_id: str) -> bool:
    """A multi-select option label is useless if it's blank, a bare number
    (the checked value, e.g. '1'), or just the raw variable code (e.g. 'Q1r1'
    or 'Q1r1 - Checked'). In those cases we should look up the real text."""
    text = str(label).strip()
    if not text:
        return True
    # bare number like "1", "0", "1.0"
    try:
        float(text)
        return True
    except (TypeError, ValueError):
    ...
    if base == str(sub_column_id):
        return True
    return False


def _resolve_ms_option_label(
    question: Any, sub_column_id: str, payload: dict[str, Any], column: Any
) -> str:
    """Best human-readable label for a multi-select / grid-of-binaries option.

    Multi-select sub-columns often arrive with a useless label (the checked
    value '1', or the raw code 'Q1r1'). Resolve the real option text from the
    schema's grid_row_labels / option_map (populated from the data map), then
    fall back to the column header, then the raw id.
    """
    payload_label = str(payload.get("label", "") or "")
    if not _label_is_uninformative(payload_label, sub_column_id):
        return payload_label

    row_labels = getattr(question, "grid_row_labels", None) or {}
    candidate = row_labels.get(sub_column_id)
    if candidate and not _label_is_uninformative(str(candidate), sub_column_id):
        return str(candidate)

    option_map = getattr(question, "option_map", None) or {}
    candidate = option_map.get(sub_column_id)
    if candidate and not _label_is_uninformative(str(candidate), sub_column_id):
        return str(candidate)
    ...
    if header:
        ...
        # only use the header if it's more than the raw code
        if base and not _label_is_uninformative(header, sub_column_id):
            return header
        # strip a trailing "- Checked" but keep anything informative
        if " - " in header and not _label_is_uninformative(
            header.rsplit(" - ", 1)[0], sub_column_id
        ):
            return header.rsplit(" - ", 1)[0].strip()

    # Last resort: the payload label (even if just the code) so we never crash.
    return payload_label or str(sub_column_id)
...
def _write_total_respondents_row(
    ...
    if total_formula:
        # Live count that follows the active filters (shows e.g. 754 when
        # filtered, 1100 unfiltered) rather than a static snapshot.
        _live_formula(
            worksheet, row_index, 2, total_formula, denominator
        ).number_format = "#,##0"
    else:
