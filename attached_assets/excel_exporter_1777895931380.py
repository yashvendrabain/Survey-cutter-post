...
def export_single_cuts(
    ...
    _write_run_summary(workbook, schema, quality_report, results, skips, log, formats)
    _write_question_metadata(workbook, schema, formats)
    _write_single_cut_index(workbook, results, sc_sheet_names, formats)
    _write_skip_log(workbook, skips, formats)
    for result in results:
    ...
    _write_filter_log(workbook, formats)
    _write_data_quality(workbook, quality_report, formats)
    _write_warnings(workbook, quality_report, results, skips, formats)
    workbook.close()
...
def _write_skip_log(
    workbook: Any, skips: list[SkipRecord], formats: dict[str, Any]
) -> None:
    ws = workbook.add_worksheet("Skip_Log")
    headers = ["Question ID", "Canonical ID", "Type", "Skip Reason", "Details"]
    _write_header_row(ws, 0, headers, formats)
...
def _write_filter_log(workbook: Any, formats: dict[str, Any]) -> None:
