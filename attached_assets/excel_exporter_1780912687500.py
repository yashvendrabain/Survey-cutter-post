...
def _live_write_question_block(
    ...
    if cross_tab_rows:
        cross_tab_start_row = (
            row_index
            if _cross_tab_can_render_side_by_side(result, question)
            else next_row + 2
        )
...
def _live_write_rank_order_table(
    ...
    worksheet.cell(row=header_row, column=3, value="Net Rank Score").font = _live_font(bold=True)
    worksheet.cell(row=subheader_row, column=3, value="0-100").font = _live_font(bold=True)
    col_index = 4
    ...
    for offset, row in enumerate(result.rows):
        ...
        for rank_number, (count, pct) in enumerate(zip(row.counts_per_rank, row.pcts_per_rank), start=1):
            ...
            col += 2
        points_terms = [
            f"{count_ref}*{result.K - rank_number + 1}"
            for rank_number, count_ref in row_count_refs
        ]
        score_formula = (
            f'=IFERROR((SUM({",".join(points_terms)})/({result.K}*$B${total_row}))*100,0)'
            if points_terms
            else "=0"
        )
        _live_formula(
...
def _cross_tab_metric_subheader(metric_kind: str) -> str:
    ...
    if metric_kind == "rank_mean":
        return "Mean rank"
    if metric_kind == "net_rank":
        return "Net Rank Score"
...
def _live_cross_tab_metric_expression(
    ...
    *,
    ...
    if metric_kind == "net_rank":
        ...
        if rank_k <= 0:
            return '""'
        ranked_count = _countifs_expression(
...
def _live_distribution_rows(
    ...
    if isinstance(result, SingleSelectResult):
        ...
        for row in result.rows:
            ...
            if column is None:
                continue
            net_rank_score = float(getattr(row, "net_rank_score", 0.0) or 0.0)
            rows.append(
                {
                    "label": row.option_label,
                    "data_name": column.data_name,
                    "criteria": ">=1",
                    "metric_kind": "net_rank",
                    "question_id": result.question_id,
                    "source_column": row.option_id,
                    "cached_value": net_rank_score,
                    "cached_total": net_rank_score,
                    "sort_count": int(round(net_rank_score * 1000)),
                    "denominator_data_names": denominator_data_names,
