...
def apply_grid_categorical_row_matching(
    ...
    if not 2 <= len(question["options"]) <= 10:
        return question
    if not _options_look_like_rating_scale(question["options"]):
        return question
...
def _options_look_like_rating_scale(options: list[tuple[int | str, str]]) -> bool:
    matched = [
        label
        for _code, label in options
        if _label_matches_rating_scale_token(str(label))
    ]
    return len(matched) >= 3


def _label_matches_rating_scale_token(label: str) -> bool:
    normalized = _normalise_rating_label(label)
    return any(
        re.search(pattern, normalized, re.IGNORECASE)
        for pattern in _RATING_TOKEN_PATTERNS
    )


def _normalise_rating_label(label: str) -> str:
    normalized = str(label).replace("\u2019", "'").casefold()
...
def _looks_like_unmatched_question_column(canonical_id: str, raw_column: str) -> bool:
    ...
    if re.match(rf"^{re.escape(canonical_id)}r\d+", text, re.IGNORECASE):
        return False
    return bool(
        re.match(rf"^{re.escape(canonical_id)}(?:\s|$)", text, re.IGNORECASE)
    )
