...
from src.ui.wizard import (
    ...
    filter_config_from_wizard_state,
...
class TestWizardHelpers(unittest.TestCase):
    ...
    def test_filter_config_from_wizard_state_preserves_custom_counts(self) -> None:
        config = filter_config_from_wizard_state(
            {
                "wizard_workbook_custom_filter_count": 5,
                "wizard_per_question_filter_count": 3,
            }
        )

        self.assertEqual(config.workbook_custom_filter_count, 5)
        self.assertEqual(config.per_question_filter_count, 3)
