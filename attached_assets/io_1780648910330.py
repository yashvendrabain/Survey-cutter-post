...
def load_survey_inputs(
    ...
    *,
    ...
    if scenario == "B_combined_xlsx":
        ...
        return _load_scenario_b(xlsx_files[0], min_questions=min_questions)
    return _load_scenario_a(uploaded_files, min_questions=min_questions)
...
def _probe_xlsx_for_scenario(file: UploadedFile) -> str:
    ...
    try:
        sheets = [
            _normalise_sheet_name_for_keywords(sheet_name)
    ...
    has_data = any(
        any(
            _normalise_sheet_name_for_keywords(keyword) in sheet_name
            for keyword in DATA_SHEET_KEYWORDS
        )
    ...
    has_map = any(
        any(
            _normalise_sheet_name_for_keywords(keyword) in sheet_name
            for keyword in MAP_SHEET_KEYWORDS
        )
        for sheet_name in sheets
    )
...
def _load_scenario_a(
    ...
    *,
    ...
    try:
        data_map = parse_datamap(dm_path, min_questions=min_questions)
...
def _load_scenario_b(
    ...
    *,
    ...
    dm_path = _write_bytes_to_temp(content, ".xlsx")
...
def _identify_combined_sheets(sheets: list[str]) -> tuple[str, str]:
    ...
    map_sheet = None
    for sheet_name in sheets:
        lowered = _normalise_sheet_name_for_keywords(sheet_name)
        if map_sheet is None and any(
            _normalise_sheet_name_for_keywords(keyword) in lowered
            for keyword in MAP_SHEET_KEYWORDS
        ):
            map_sheet = sheet_name

    for sheet_name in sheets:
        if sheet_name == map_sheet:
            continue
        lowered = _normalise_sheet_name_for_keywords(sheet_name)
        if data_sheet is None and any(
            _normalise_sheet_name_for_keywords(keyword) in lowered
            for keyword in RAW_NAME_KEYWORDS
        ):
    ...
    if data_sheet is None:
        data_sheet = next((sheet for sheet in sheets if sheet != map_sheet), sheets[0])
...
def _name_contains(name: str, keywords: set[str]) -> bool:
    lowered = _normalise_sheet_name_for_keywords(name)
    return any(
        _normalise_sheet_name_for_keywords(keyword) in lowered
...
def _normalise_sheet_name_for_keywords(name: str) -> str:
