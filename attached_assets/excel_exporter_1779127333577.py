...
def _live_write_grid_rated_table(
    ...
    for offset, entry in enumerate(row_entries):
        ...
        if has_delta and delta_start_col is not None and len(group_cells) >= 2:
            mean_formula = f"={group_cells[0][0]}-{group_cells[1][0]}"
            median_formula = f"={group_cells[0][1]}-{group_cells[1][1]}"
            ...
            _live_formula(
                ...
                delta_start_col,
                mean_formula,
                float(first_stats.get("mean", 0.0)) - float(second_stats.get("mean", 0.0)),
            ).number_format = "0.0"
            _live_formula(
                worksheet,
                excel_row,
                delta_start_col + 1,
                median_formula,
                float(first_stats.get("median", 0.0)) - float(second_stats.get("median", 0.0)),
            ).number_format = "0.0"
...
def _live_write_grid_categorical_table(
    ...
    for offset, category in enumerate(categories, start=3):
        worksheet.cell(row=header_row, column=offset, value=category).font = _live_font(bold=True)
        worksheet.cell(row=subheader_row, column=offset, value="Count").font = _live_font(bold=True)
    last_col = max(4, len(categories) + 2)
    for row in (header_row, subheader_row):
        for col in range(1, last_col + 1):
            worksheet.cell(row=row, column=col).fill = _live_fill("F2F2F2")
    ...
    for offset, (sub_column_id, row_result) in enumerate(result.rows.items()):
        ...
        for category_index, category in enumerate(categories, start=3):
            ...
            if category == "Other":
                ...
                top_refs = ",".join(
                    f"{_openpyxl_column_letter(col)}{excel_row}"
                    for col in range(3, 3 + len(top_categories))
                )
    ...
    if result.rows:
        for col in range(3, len(categories) + 3):
            _apply_color_scale_range(worksheet, data_start, col, data_end, col)
    total_row = max(data_start, data_end + 1)
