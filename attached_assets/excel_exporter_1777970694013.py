...
def _create_workbook(output_path: str) -> Any:
    if xlsxwriter is not None:
        return xlsxwriter.Workbook(output_path, {"nan_inf_to_errors": True})
...
def _write_cc_sheet(
    ...
    if result.analysis_type is AnalysisType.CROSS_TAB:
        last_row = _write_cross_tab_body(ws, result.result_table, formats, 7)
    elif result.analysis_type is AnalysisType.SEGMENT_PROFILE:
        last_row = _write_segment_profile_body(ws, result.result_table, formats, 7)
    elif result.analysis_type is AnalysisType.GROUP_COMPARISON:
        last_row = _write_group_comparison_body(ws, result.result_table, formats, 7)
    elif result.analysis_type is AnalysisType.EXPECTED_VS_REALIZED:
        last_row = _write_expected_vs_realized_body(ws, result.result_table, formats, 7)
    else:
...
def _write_segment_profile_body(
    ...
    _write(ws, start_row, 0, f"Segment: {result_table.get('filter_expr', '')}", formats["bold"])
    _write(ws, start_row + 1, 0, f"Filter N: {result_table.get('filter_n', 0)}", formats["bold"])
    _write(ws, start_row + 2, 0, "Target distribution", formats["bold"])
...
def _write_serialized_single_cut_body(
    ...
    if "distribution" in payload:
        return _write_distribution_dict(
            ws, payload.get("distribution", {}), formats, start_row
        )
...
def _write_group_comparison_body(
    ...
    _write(ws, start_row, 0, "Per-segment comparison", formats["bold"])
    _write_header_row(ws, start_row + 1, ["Segment", "Label", "N", "Mean", "Median", "Std"], formats)
...
def _write_expected_vs_realized_body(
    ...
    _write(ws, start_row, 0, "Expected vs Realized", formats["bold"])
    _write_header_row(ws, start_row + 1, ["Metric", "Expected", "Realized", "Gap"], formats)
    rows = [
