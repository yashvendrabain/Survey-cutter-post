...
def _render_wizard_top_nav() -> None:
    """Compact chevron nav with the 3 step circles (Categories / Filter counts /
    Cross-cut) centered neatly between the ‹ › arrows."""
    app = _require_streamlit()
    step = int(app.session_state.get("wiz_step", 1))
    ...
    with mid_col:
        ...
        # Circle styling — tight, centered, connected by a hairline; each circle
        # sits directly above its label so it reads as a clean stepper.
        app.markdown(
            "<style>"
            ".wiztop-row + div [data-testid='stHorizontalBlock']{"
            "max-width:520px;margin:0 auto;gap:0 !important;align-items:start;}"
        ...
        # circle | connector | circle | connector | circle
        cc = app.columns([2, 1, 2, 1, 2])
        circle_cols = [cc[0], cc[2], cc[4]]
        line_cols = [cc[1], cc[3]]
        for li, lc in enumerate(line_cols):
            done = (li + 1) < step
            lc.markdown(
                f"<div style='height:34px;display:flex;align-items:center;'>"
                f"<div style='height:2px;width:100%;background:"
                f"{'#CC0000' if done else '#E2E4E8'};'></div></div>",
                unsafe_allow_html=True,
            )
        for idx, (col, label) in enumerate(zip(circle_cols, labels), start=1):
            with col:
                if app.button(
                    ...
                    _wizard_rerun(app)
                weight = "700" if idx == step else "500"
                color = "#CC0000" if idx == step else "#999"
                app.markdown(
                    f"<div style='text-align:center;font-size:11px;font-weight:{weight};"
                    f"color:{color};font-family:Arial,sans-serif;margin-top:4px;white-space:nowrap;'>{label}</div>",
                    unsafe_allow_html=True,
                )
    if step < 3:
        if next_col.button(
