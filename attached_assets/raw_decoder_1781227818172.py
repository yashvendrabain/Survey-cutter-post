...
def decode_raw_data(
    ...
    grid_patterns = _grid_patterns_for_decoding(data_map, raw_columns)
    value_ranges = _value_ranges_by_column(data_map)
    no_numeric_coercion_columns = _string_preserved_columns(data_map, grid_patterns)
    ...
    dataframe = _decode_option_columns(dataframe, data_map, grid_patterns)
...
def _string_preserved_columns(
    data_map: DataMap,
    grid_patterns: dict[str, str] | None = None,
) -> set[str]:
    grid_patterns = grid_patterns or {}
    preserved: set[str] = set()
    for question in data_map["questions"]:
        if question["type_hint"] == "open_text" or (
            question["type_hint"] == "open_numeric"
            and question["canonical_id"] not in grid_patterns
        ):
...
_GRID_CATEGORICAL_ROW = "grid_categorical_row"
_GRID_RATED_DOUBLE_COLON = "grid_rated_double_colon"
_TRAILING_LABEL_PUNCTUATION = ".,;:!?- "


def _grid_patterns_for_decoding(
    data_map: DataMap,
    raw_columns: tuple[str, ...],
) -> dict[str, str]:
    patterns: dict[str, str] = {}
    for question in data_map["questions"]:
        pattern = _detect_grid_pattern_for_decoding(question, raw_columns)
        if pattern is not None:
            patterns[question["canonical_id"]] = pattern
    return patterns


def _detect_grid_pattern_for_decoding(
    question: ParsedQuestion,
    raw_columns: tuple[str, ...],
) -> str | None:
    if _has_double_colon_grid_columns(question, raw_columns):
        return _GRID_RATED_DOUBLE_COLON
    if _has_categorical_grid_row_columns(question, raw_columns):
        return _GRID_CATEGORICAL_ROW
    return None


def _has_double_colon_grid_columns(
    question: ParsedQuestion,
    raw_columns: tuple[str, ...],
) -> bool:
    sub_column_labels = _sub_column_labels(question)
    if len(sub_column_labels) >= 2:
        return any("::" in label for label in sub_column_labels)
    ...
    for raw_column in raw_columns:
        ...
        if not dim2_label or _is_grid_helper_label(dim1_label):
            continue
        cells.append((dim1_label, dim2_label))
    return len(cells) >= 2


def _has_categorical_grid_row_columns(
    question: ParsedQuestion,
    raw_columns: tuple[str, ...],
) -> bool:
    ...
    if not 2 <= len(options) <= 10:
        return False
    if _grid_options_look_binary_selection(options):
        return False

    sub_column_labels = _sub_column_labels(question)
    if len(sub_column_labels) >= 2:
        return not _labels_match_options(sub_column_labels, options)

    canonical_id = question["canonical_id"]

    row_labels = _infer_grid_row_labels(canonical_id, raw_columns)
    if len(row_labels) < 2:
        return False
    return not _labels_match_options(row_labels, options)


def _sub_column_labels(question: ParsedQuestion) -> list[str]:
    labels: list[str] = []
    sub_columns = question.get("sub_columns", []) or []
    for _column, label in sub_columns:
...
def _labels_match_options(
    ...
    if not labels:
        return False
    matches = sum(1 for label in labels if _label_matches_option(label, options))
    return matches / len(labels) >= threshold


def _infer_grid_row_labels(
    canonical_id: str,
    raw_columns: tuple[str, ...],
) -> list[str]:
    ...
    used: set[str] = set()
    for raw_column in raw_columns:
        match = row_pattern.match(str(raw_column))
        if match is None:
            continue
...
def _label_matches_option(label: str, options: list[tuple[Any, Any]]) -> bool:
    normalized = _normalise_label_text(label)
    for _code, option_label in options:
        option_text = str(option_label).strip()
        if option_text == str(label).strip():
            return True
        if option_text.casefold() == str(label).strip().casefold():
            return True
        option_norm = _normalise_label_text(option_text)
        if option_norm == normalized:
            return True
        if normalized and option_norm and (
            normalized in option_norm or option_norm in normalized
        ):
            return True
    return False
...
def _normalise_label_text(value: str) -> str:
    ...
    stripped = without_parentheticals.rstrip(_TRAILING_LABEL_PUNCTUATION)
    return re.sub(r"\s+", " ", stripped).strip().casefold()


def _grid_options_look_binary_selection(options: list[tuple[Any, Any]]) -> bool:
    normalized_labels = {_normalise_label_text(str(label)) for _code, label in options}
    if len(normalized_labels) > 3:
        return False
    selected_labels = {"selected", "checked", "yes", "true"}
    unselected_labels = {
        "not selected",
        "unselected",
        "unchecked",
    ...
    return bool(normalized_labels & selected_labels) and bool(
        normalized_labels & unselected_labels
    )


def _is_grid_helper_label(label: str) -> bool:
    normalized = str(label).replace("\u2019", "'").strip().casefold()
...
def _decode_option_columns(
    ...
    for question in data_map["questions"]:
        if question.get("label_to_numeric_value"):
            continue
        pattern = grid_patterns.get(question["canonical_id"])
        if pattern in {_GRID_CATEGORICAL_ROW, _GRID_RATED_DOUBLE_COLON}:
            continue
        if _is_grid_single_select_question(question):
            decoded = _decode_text_selection_sub_columns(decoded, question)
            continue
...
def _decode_text_selection_sub_columns(
    dataframe: pd.DataFrame,
    question: ParsedQuestion,
) -> pd.DataFrame:
    decoded = dataframe.copy()
    ...
    for column, _fallback_label in question["sub_columns"]:
        ...
        for value in decoded[column].tolist():
            ...
            if (
                selected == 1
                and clean_text
                and not _is_bare_number(clean_text)
                and column not in discovered_labels
            ):
...
def _decode_grid_cell_value(value: Any) -> tuple[int | None, str]:
    ...
    if isinstance(value, str):
        ...
        # A bare number (e.g. "1"/"0", the checked-value code) is a selection
        # indicator, never an option label. Returning it as clean_text would let
        # it overwrite the real data-map label (e.g. "United States") downstream.
        if _is_bare_number(clean_text):
            return (0 if was_rejected else 1), ""
        return (0 if was_rejected else 1), clean_text
    # Numeric / boolean cells carry the selection flag only — never a label.
    return (1 if _truthy_grid_value(value) else 0), ""
