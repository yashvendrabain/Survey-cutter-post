...
def _option_choice_chart(work_df, mask_win, mask_lag, mask_oth, question_col, title, key):
    """Grouped bar: % of Winners / Laggards / Others choosing each option.

    Winners red, Laggards black, Others grey. Percentage is within each group
    (denominator = that group's count of people who answered this question).
    """
    try:
    ...
    fig = go.Figure()
    fig.add_bar(name="Winners", x=options, y=win_pct, marker_color="#CC0000",
                text=[f"{v:.0f}%" for v in win_pct], textposition="outside")
    fig.add_bar(name="Laggards", x=options, y=lag_pct, marker_color="#1A1A1A",
    ...
    fig.update_layout(barmode="group", height=340, title=title,
                      font_family="Arial", margin=dict(t=50, b=30),
...
def render_advanced_outcome_segmentation() -> None:
    ...
    # ---- Winner / Laggard UID lists (counts + downloads) ----
    st.markdown("#### Winner & Laggard respondents")
    ccol1, ccol2, ccol3 = st.columns(3)
    ccol1.metric("Winners", int(mask_win.sum()))
    ccol2.metric("Laggards", int(mask_lag.sum()))
    ...
    dl1.download_button("Download winner IDs", "\n".join(win_ids),
                        file_name="winner_ids.csv", key="adv_dl_win",
                        use_container_width=True, disabled=not win_ids)
    dl2.download_button("Download laggard IDs", "\n".join(lag_ids),
                        file_name="laggard_ids.csv", key="adv_dl_lag",
                        use_container_width=True, disabled=not lag_ids)
    with st.expander("Show winner / laggard ID lists", expanded=False):
        import pandas as pd
        maxlen = max(len(win_ids), len(lag_ids))
        st.dataframe(pd.DataFrame({
            "Winner IDs": win_ids + [""] * (maxlen - len(win_ids)),
            "Laggard IDs": lag_ids + [""] * (maxlen - len(lag_ids)),
        }), use_container_width=True, hide_index=True)

    # ---- Per-measure charts: Winners / Laggards / Others ----
    ...
    for spec in specs:
        ...
        if breakdown_dim is None:
            _option_choice_chart(work_df, mask_win, mask_lag, mask_oth, spec.column,
                                 f"{spec.column} — Winners vs Laggards vs Others",
