...
def _build_filters_sheet(
    ...
    headers = ["Filter", "Value", "Resolved criterion", "Raw data column"]
    _live_header_row(worksheet, 2, headers)
    row_index = 3
    ...
    for question in demographic_questions[:8]:
        ...
        crit_cell = worksheet.cell(
            row=row_index,
            column=3,
            value=f'=IF({filter_name}="(All)","*",{filter_name})',
        )
        worksheet.cell(row=row_index, column=4, value=column.header)
        _add_named_cell(workbook, filter_name, worksheet, value_cell.coordinate)
        _add_named_cell(workbook, f"{filter_name}_crit", worksheet, crit_cell.coordinate)
        _add_dropdown_to_cell(worksheet, value_cell.coordinate, f"={column.header}_options")
        sheet_filters.append(
            {
                "filter_name": filter_name,
                "crit_name": f"{filter_name}_crit",
    ...
    row_index += 2
    worksheet.cell(row=row_index, column=1, value="PER-QUESTION FILTERS").font = _live_font(bold=True)
    _live_fill_row(worksheet, row_index, 7, "F2F2F2")
    row_index += 1
    _live_header_row(
        worksheet,
        row_index,
        ["Question", "Filter question", "Filter value", "Cross-tab by", "Resolved column", "Resolved criterion"],
    )
    row_index += 1
    ...
    for result in results:
        ...
        fv_cell = worksheet.cell(row=row_index, column=3, value="(All)")
        ct_cell = worksheet.cell(row=row_index, column=4, value="(None)")
        resolved_column = worksheet.cell(
            row=row_index,
            column=5,
            value=f'=IF({fq_name}="(None)","respondent_id_data",{fq_name} & "_data")',
        )
        resolved_criterion = worksheet.cell(
            row=row_index,
            column=6,
            value=f'=IF({fq_name}="(None)","<>",IF({fv_name}="(All)","*",{fv_name}))',
        )
        ...
        _add_named_cell(workbook, f"{q_prefix}_F_resolved_column", worksheet, resolved_column.coordinate)
        _add_named_cell(workbook, f"{q_prefix}_F_crit", worksheet, resolved_criterion.coordinate)
...
def _live_write_custom_filter_slot(
    ...
    resolved_column = worksheet.cell(
        row=row_index,
        column=col_index + 4,
        value=f'=IF({question_name}="(None)","respondent_id_data",{question_name} & "_data")',
    )
    resolved_criterion = worksheet.cell(
        row=row_index,
        column=col_index + 5,
        value=f'=IF({question_name}="(None)","<>",IF({value_name}="(All)","*",{value_name}))',
    )
    ...
    _add_named_cell(workbook, f"{prefix}_crit", worksheet, resolved_criterion.coordinate)
...
def _filter_formula_pairs(sheet_filters: list[dict[str, str]]) -> list[tuple[str, str]]:
    return [
        (
            ...
            sheet_filter.get("crit_name", f'{sheet_filter["filter_name"]}_crit'),
        )
...
def _dynamic_filter_pairs(question_name: str, value_name: str) -> list[tuple[str, str]]:
    ...
    return [
        (
            f"INDIRECT({prefix}_resolved_column)",
            f"{prefix}_crit",
        )
    ]
