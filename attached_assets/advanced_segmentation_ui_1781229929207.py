...
def _subcolumn_ids_from_schema():
    """All sub-column ids that belong to a multi-row parent question (grid rows,
    multi-select options, rank items). These are individual rows like Q15r1,
    Q15r2 — they must NOT appear as standalone metric/measure options."""
    schema = st.session_state.get("schema")
    sub_ids = set()
    if schema is None:
        return sub_ids
    for spec in getattr(schema, "questions", []) or []:
        raw_cols = tuple(getattr(spec, "raw_columns", ()) or ())
        canonical = getattr(spec, "canonical_id", None)
        # A standalone question has exactly one raw column equal to its id.
        # Anything else (grids, multi-selects, rank, NPS multi-entity) exposes
        # per-row sub-columns we should hide from the measure pickers.
        if len(raw_cols) > 1 or (len(raw_cols) == 1 and raw_cols[0] != canonical):
            for col in raw_cols:
                if col != canonical:
                    sub_ids.add(col)
        # grid_row_labels keys are sub-columns too (belt and suspenders)
        grl = getattr(spec, "grid_row_labels", None) or {}
        sub_ids.update(grl.keys())
    return sub_ids


def _dimension_eligible_cols(df, cols):
    """Columns usable as a sector/region split: standalone categorical questions
    with a small number of distinct values. Excludes grid/multi-select
    sub-columns (Q15r1, ...) and high-cardinality / mostly-numeric columns."""
    import pandas as pd
    sub_ids = _subcolumn_ids_from_schema()
    out = []
    seen = set()
    for c in cols:
        if c in seen or c in sub_ids:
            continue
        series = df[c].dropna()
        if series.empty:
            continue
        n_unique = series.nunique()
        # categorical-ish: few distinct values, not a free-form / numeric scalar
        if 1 < n_unique <= 50:
            out.append(c)
            seen.add(c)
    return out
...
def _metric_eligible_cols(df, cols):
    ...
    Excludes (a) exploded multi-select OPTION columns (0/1 flags) and (b) grid
    sub-columns / rank items (e.g. Q15r1, Q15r2, ...) — both were showing up as
    repeated 'Q15: ... / Q15r1 / Q15r2 / ...' entries and aren't meaningful
    single metrics. Dedupes while preserving order.
    """
    import pandas as pd
    sub_ids = _subcolumn_ids_from_schema()
    out = []
    ...
    for c in cols:
        if c in seen:
            continue
        if c in sub_ids:
            continue  # skip grid/multi-select sub-columns
        series = df[c]
...
def render_advanced_outcome_segmentation() -> None:
    ...
    dim_cols = _dimension_eligible_cols(df, cols)
    ...
    default_dims = _guess(dim_cols, _DIM_HINTS, n=2)
    ...
    with box_sec:
        with st.container(border=True):
            ...
            balance_dims = st.multiselect(
                "Dimensions", dim_cols, default=default_dims, key="adv_seg_dims",
