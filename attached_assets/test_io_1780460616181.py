...
from src.io import (
    ...
    _safe_unlink,
    _write_datamap_sheet_as_sheet1,
    load_survey_inputs,
)
...
class TestIO(unittest.TestCase):
    ...
    def test_scenario_b_skips_rewrite_when_sheet_name_recognized(self) -> None:
        with patch(
            "src.io._write_datamap_sheet_as_sheet1",
    ...
    def test_scenario_b_still_works_with_unrecognized_sheet_name(self) -> None:
        with patch(
            "src.io._write_datamap_sheet_as_sheet1",
            side_effect=AssertionError("rewrite should not be called"),
        ):
    ...
    def test_scenario_b_load_produces_same_datamap_as_before(self) -> None:
        content = COMBINED_XLSX_PATH.read_bytes()
        old_dm_path = _write_datamap_sheet_as_sheet1(content, "DataMap")
        try:
            old_data_map = parse_datamap(old_dm_path)
        finally:
            _safe_unlink(old_dm_path)

        new_data_map, _raw_df, _report = load_survey_inputs(
            [upload_from_path(COMBINED_XLSX_PATH, "combined.xlsx")]
        )

        self.assertEqual(new_data_map["questions"], old_data_map["questions"])
        self.assertEqual(
            new_data_map["parser_warnings"],
            old_data_map["parser_warnings"],
        )
