...
class TestOutcomeSegmentation(unittest.TestCase):
    ...
    def test_multi_select_rates_use_cohort_answered_denominator(self) -> None:
        winner_n = 100
        laggard_n = 100
        ...
        def option_values(
            winner_answered: int,
            winner_selected: int,
            laggard_answered: int,
            laggard_selected: int,
        ) -> list:
            values = [None] * rows
            values[:winner_answered] = [1] * winner_selected + [0] * (
                winner_answered - winner_selected
            )
            values[winner_n : winner_n + laggard_answered] = [1] * laggard_selected + [
                0
            ] * (laggard_answered - laggard_selected)
            return values

        df = pd.DataFrame(
            {
                "Q_OUTCOME": [1] * winner_n + [2] * laggard_n,
                "Q4r1": option_values(70, 42, 84, 39),
                "Q5r1": option_values(68, 11, 79, 7),
                "Q4_REVr1": option_values(70, 48, 84, 56),
            }
        )
        schema = _schema(
            [
                ...
                _question(
                    "Q4_REV",
                    QuestionType.MULTI_SELECT_BINARY,
                    raw_columns=("Q4_REVr1",),
                    option_map={"Q4_REVr1": "Revenue intelligence"},
                ),
            ]
        )
        ...
        self.assertAlmostEqual(by_id["Q4"].top_option_winner_rate * 100, 60.0, places=4)
        self.assertAlmostEqual(
            by_id["Q4"].top_option_loser_rate * 100,
            46.4286,
            places=4,
        )
        self.assertEqual(by_id["Q4"].winner_n, 70)
        self.assertEqual(by_id["Q4"].loser_n, 84)
        self.assertAlmostEqual(
            by_id["Q5"].top_option_winner_rate * 100,
            16.1765,
            places=4,
        )
        self.assertAlmostEqual(
            by_id["Q5"].top_option_loser_rate * 100,
            8.8608,
            places=4,
        )
        self.assertAlmostEqual(by_id["Q5"].top_option_lift, 1.83, delta=0.01)
        revenue_gap = (
            by_id["Q4_REV"].top_option_winner_rate
            - by_id["Q4_REV"].top_option_loser_rate
        )
        self.assertAlmostEqual(
            by_id["Q4_REV"].top_option_winner_rate * 100,
            68.5714,
            places=4,
        )
        self.assertAlmostEqual(
            by_id["Q4_REV"].top_option_loser_rate * 100,
            66.6667,
            places=4,
        )
        self.assertGreater(revenue_gap, 0.015)
