...
- If no clear standout exists, still write a grounded descriptive
  summary using the table's numbers: the dominant response and share,
  the distribution shape, or the highest and lowest items. Never emit
  a null/no-differentiation message.
...
TEMPLATE_INSIGHT = "Review the computed table for the leading response and distribution shape."
...
def _call_openai(
    *,
    ...
    if _is_null_insight(headline):
        return _template_fallback(
            table_payload,
            table_kind,
            title_hint,
            error_message="AI returned a null/no-differentiation message",
        )
...
def _is_null_insight(text: str) -> bool:
    normalized = re.sub(r"\s+", " ", str(text or "").strip().lower())
    return (
        "no strong differentiation visible" in normalized
        or normalized in {
...
def _template_fallback(
    ...
    return InsightResult(
        title="",
        insight=_descriptive_summary_fallback(payload, table_kind, title_hint),
        was_template=True,
...
def _descriptive_summary_fallback(
    payload: dict,
    table_kind: str,
    title_hint: str,
) -> str:
    if table_kind in {"single_cut", "filtered_single_cut"}:
        return _single_cut_summary_fallback(payload, title_hint)
    key_finding = str(payload.get("key_finding", "") or "").strip()
    if key_finding:
        return key_finding
    rows = payload.get("rows")
    if isinstance(rows, list) and rows:
        row = _row_with_largest_numeric_value(rows)
        if row is not None:
            label, value = row
            return f"{label} has the largest computed value at {_format_fallback_number(value)}."
    return TEMPLATE_INSIGHT


def _single_cut_summary_fallback(payload: dict, title_hint: str) -> str:
    rows = payload.get("rows")
    if not isinstance(rows, list) or not rows:
        summary = payload.get("summary")
        if isinstance(summary, dict) and summary:
            first_key, first_value = next(iter(summary.items()))
            return (
                f"{title_hint or payload.get('question_text', 'This cut')} "
                f"has {first_key} {_format_fallback_number(first_value)}."
            )
        return TEMPLATE_INSIGHT

    ranked: list[tuple[str, float, int | None, float | None]] = []
    for row in rows:
        if not isinstance(row, dict):
            continue
        label = str(row.get("label") or row.get("option") or row.get("name") or "Response")
        count = _safe_float(row.get("count"))
        rate = _safe_float(row.get("rate"))
        score = rate if rate is not None else count
        if score is None:
            continue
        ranked.append((label, float(score), int(count) if count is not None else None, rate))
    if not ranked:
        return TEMPLATE_INSIGHT
    ranked.sort(key=lambda item: item[1], reverse=True)
    top_label, _score, top_count, top_rate = ranked[0]
    valid_n = payload.get("valid_n")
    if top_rate is not None and 0 <= top_rate <= 1:
        if top_count is not None:
            return f"{top_label} leads the cut at {top_rate:.0%} ({top_count} responses)."
        return f"{top_label} leads the cut at {top_rate:.0%}."
    if top_count is not None:
        suffix = f" out of {valid_n}" if isinstance(valid_n, int) and valid_n else ""
        return f"{top_label} leads the cut with {top_count} responses{suffix}."
    return f"{top_label} is the largest response in this cut."


def _row_with_largest_numeric_value(rows: list[Any]) -> tuple[str, float] | None:
    best: tuple[str, float] | None = None
    for row in rows:
        if not isinstance(row, dict):
            continue
        label = str(row.get("label") or row.get("question") or row.get("option") or "Row")
...
def _safe_float(value: Any) -> float | None:
    if value is None or isinstance(value, bool):
        return None
    try:
        return float(str(value).strip().rstrip("%xX").replace(",", ""))
    except (TypeError, ValueError):
        return None


def _format_fallback_number(value: Any) -> str:
    numeric = _safe_float(value)
    if numeric is None:
        return str(value)
