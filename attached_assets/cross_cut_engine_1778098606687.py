...
def _compute_cross_tab(
    ...
    column_a = _simple_categorical_column(question_a, df)
    column_b = _simple_categorical_column(question_b, df)
...
def _compute_group_comparison(
    ...
    segment_column = _simple_categorical_column(segment_spec, df)
...
def _simple_categorical_column(
    ...
    if question_spec.question_type in (
        QuestionType.SINGLE_SELECT,
        QuestionType.DEMOGRAPHIC_OR_SEGMENT,
    ):
        if question_spec.canonical_id not in df.columns:
            raise ValueError(f"{question_spec.canonical_id} not in raw data")
        return question_spec.canonical_id

    if question_spec.question_type is QuestionType.GRID_SINGLE_SELECT:
        if question_spec.canonical_id in df.columns:
            return question_spec.canonical_id
        if question_spec.raw_columns and question_spec.raw_columns[0] in df.columns:
            return question_spec.raw_columns[0]
        raise ValueError(
            f"{question_spec.canonical_id}: no usable column in raw data"
        )

    raise ValueError(
        f"{question_spec.canonical_id} is not categorical: "
        f"{question_spec.question_type}"
    )
