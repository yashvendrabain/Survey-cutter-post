...
class QuestionSpec:
    ...
    label_to_numeric_value: dict[str, float] = field(default_factory=dict)
    na_label_set: frozenset[str] = frozenset()
    allowed_numeric_range: tuple[float, float] | None = None
...
class HypothesisGroup:
    """One side of a categorical predictor."""

    label: str
    matching_values: tuple[int | str, ...]

    def __post_init__(self) -> None:
        _require_non_empty_string(self.label, "label")
        if not self.matching_values:
            raise ValueError("matching_values must not be empty")


@dataclass(frozen=True, slots=True)
class HypothesisSpec:
    """Structured hypothesis input."""

    title: str
    outcome_question_id: str
    predictor_question_id: str
    outcome_direction: str
    predictor_groups: tuple[HypothesisGroup, ...] = ()
    free_text: str = ""

    def __post_init__(self) -> None:
        _require_non_empty_string(self.title, "title")
        _require_non_empty_string(self.outcome_question_id, "outcome_question_id")
        _require_non_empty_string(self.predictor_question_id, "predictor_question_id")
        valid_directions = {
            "higher",
            "lower",
            "different",
        ...
        if self.outcome_direction not in valid_directions:
            raise ValueError(f"invalid outcome_direction: {self.outcome_direction}")


@dataclass(frozen=True, slots=True)
class HypothesisStatistic:
    """The numerical result of a selected statistical test."""
    ...
    sample_size: int
    sample_sizes_per_group: tuple[int, ...] = ()
    degrees_of_freedom: float | None = None
    raw_test_statistic: float | None = None


@dataclass(frozen=True, slots=True)
class HypothesisResult:
    """Full audit-trail-ready result for one hypothesis run."""

    spec: HypothesisSpec
    verdict: str
    verdict_reason: str
    statistic: HypothesisStatistic | None
    cohort_n: int
    pairwise_n: int
    active_filters_summary: str
    related_cross_cut_id: str | None
    calculation_log_entry_ids: tuple[str, ...]
    ai_prose_explanation: str = ""
    warnings: tuple[str, ...] = ()
...
class RankOrderRow:
    ...
    def __post_init__(self) -> None:
        ...
        _require_numeric(self.weighted_average, "weighted_average")
        if float(self.weighted_average) < 0.0:
            raise ValueError("weighted_average must be non-negative")
...
class RankOrderResult(SingleCutResult):
    ...
    def __post_init__(self) -> None:
        ...
        for row in self.rows:
            ...
            if float(row.weighted_average) > float(self.K):
                raise ValueError("weighted_average must be between 0 and K")
...
@dataclass(frozen=True, slots=True)
class NPSEntityResult:
    """NPS breakdown for one entity/brand column."""
    ...
    def __post_init__(self) -> None:
        _require_non_empty_string(self.entity_label, "entity_label")
        for field_name in ("promoters", "passives", "detractors", "valid_n", "missing_n"):
            _require_non_negative_int(getattr(self, field_name), field_name)
        _require_rate(self.pct_promoters, "pct_promoters")
        _require_rate(self.pct_passives, "pct_passives")
        _require_rate(self.pct_detractors, "pct_detractors")
        _require_numeric(self.nps_score, "nps_score")
        if self.promoters + self.passives + self.detractors != self.valid_n:
            raise ValueError(
                "promoters + passives + detractors must equal valid_n"
            )


@dataclass(frozen=True, slots=True)
class NPSResult(SingleCutResult):
    """Net Promoter Score result for one or more entity columns."""

    entities: list[NPSEntityResult]

    def __post_init__(self) -> None:
        super(NPSResult, self).__post_init__()
        if self.question_type is not QuestionType.NPS:
            raise ValueError("NPSResult requires question_type NPS")
        if not self.entities:
            raise ValueError("entities must be non-empty")
        for entity in self.entities:
            if not isinstance(entity, NPSEntityResult):
                raise ValueError("entities must contain NPSEntityResult instances")
...
class DataQualityReport:
    ...
    warnings: tuple[str, ...]
    decoder_warnings: tuple[dict, ...] = field(default_factory=tuple)
...
class SegmentDefinition:
    """Defines how to split an outcome variable into winner/laggard groups."""
    ...
    loser_label: str = "Laggard"
    quartile_winner: str = "top"
    laggard_values: tuple[Any, ...] = ()
    laggard_threshold: float | None = None
    laggard_threshold_direction: str = "lte"
    laggard_label: str = "Laggards"
    ...
    def __post_init__(self) -> None:
        ...
        if self.segment_mode not in (
            ...
            raise ValueError(
                "segment_mode must be categorical, numeric_threshold, quartile, or manual_uuid"
            )
        ...
        if self.laggard_threshold_direction not in ("gte", "lte"):
            raise ValueError("laggard_threshold_direction must be gte or lte")
        if self.quartile_winner not in ("top", "bottom"):
            raise ValueError("quartile_winner must be 'top' or 'bottom'")
...
class LoadReport:
    ...
    manual_cohort_input: Any | None = None
