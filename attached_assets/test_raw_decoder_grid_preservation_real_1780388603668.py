...
def _q27_question() -> dict:
    return {
        ...
        "sub_columns": [
            ("Q27: Field sales", "Field sales"),
            ("Q27: Inside sales", "Inside sales"),
        ],
...
class TestRawDecoderGridPreservationReal(unittest.TestCase):
    ...
    def test_q27_style_columns_preserved_numeric(self) -> None:
        decoded = self._decode(
            [
                {"respondent_id": "R1", "Q27: Field sales": 3, "Q27: Inside sales": 2},
                {"respondent_id": "R2", "Q27: Field sales": 1, "Q27: Inside sales": 5},
            ],
            _q27_question(),
        )

        self.assertEqual(decoded["Q27: Field sales"].tolist(), [3.0, 1.0])
        self.assertEqual(decoded["Q27: Inside sales"].tolist(), [2.0, 5.0])
        self.assertNotIn("Selected", decoded["Q27: Field sales"].tolist())
        self.assertNotIn("Same", decoded["Q27: Field sales"].tolist())
    ...
    def test_q14_style_double_colon_preserved_float(self) -> None:
        question = {
            ...
            "sub_columns": [
                ("Q14: Planned 2024 :: Revenue Growth", "Planned 2024 :: Revenue Growth"),
                ("Q14: Planned 2024 :: Margin Expansion", "Planned 2024 :: Margin Expansion"),
            ],
            "parent_canonical_id": None,
            "source_row": 1,
            "warnings": [],
    ...
    def test_q27_real_winvslag_simulation(self) -> None:
        decoded = self._decode(
            [
                {"respondent_id": "R1", "Q27: Field sales": 3, "Q27: Inside sales": 2},
                {"respondent_id": "R2", "Q27: Field sales": 3, "Q27: Inside sales": 4},
                {"respondent_id": "R3", "Q27: Field sales": None, "Q27: Inside sales": 5},
            ],
            _q27_question(),
        )

        self.assertEqual(decoded["Q27: Field sales"].dropna().tolist(), [3.0, 3.0])
        self.assertEqual(decoded["Q27: Inside sales"].dropna().tolist(), [2.0, 4.0, 5.0])
        self.assertNotIn("Selected", decoded["Q27: Field sales"].astype(str).tolist())
        self.assertNotIn("More often", decoded["Q27: Field sales"].astype(str).tolist())
