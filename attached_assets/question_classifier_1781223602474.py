...
_FREE_TEXT_SUBCOLUMN_TOKENS = ("user input", "other", "specify")
_METADATA_ID_EXCLUSION_REASON = (
    "metadata/ID column (near-unique values; not a survey response)"
)
_ID_REFERENCE_STEM_PATTERN = re.compile(
    r"\b("
    r"reference\s+code|respondent\s+id|respondent\s+identifier|"
    r"system[-\s]+generated|identifier|id"
    r")\b",
    re.IGNORECASE,
)
_ID_VALUE_PATTERNS = (
    re.compile(r"^[A-Za-z]{0,4}\d{3,}$"),
    re.compile(
        r"^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$",
        re.IGNORECASE,
    ),
)
_RECLASSIFIED_TO_RANK_WARNING = (
    "reclassified MULTI_SELECT_BINARY -> RANK_ORDER by value inspection"
)
_RECLASSIFIED_TO_ALLOCATION_WARNING = (
    "reclassified MULTI_SELECT_BINARY -> NUMERIC_ALLOCATION by value inspection"
)
_NPS_RECOMMEND_PATTERN = re.compile(r"\brecommend\b", re.IGNORECASE)
_NPS_SCALE_PATTERN = re.compile(
...
def reconcile_multiselect_value_subtypes(
    schema: SurveySchema,
    df: pd.DataFrame,
) -> SurveySchema:
    """Reclassify compact lookalikes using decoded response values."""

    reconciled_questions: list[QuestionSpec] = []
    changed = False
    for spec in schema.questions:
        if _should_reclassify_as_metadata_id(spec, df):
            reconciled_questions.append(
                replace(
                    spec,
                    question_type=QuestionType.METADATA_OR_ID,
                    analysis_eligible=False,
                    exclusion_reason=_METADATA_ID_EXCLUSION_REASON,
        ...
        if spec.question_type is not QuestionType.MULTI_SELECT_BINARY:
            reconciled_questions.append(spec)
            continue

        sibling_cols = _value_subtype_sibling_columns(spec, df)
        if len(sibling_cols) < 2:
            reconciled_questions.append(spec)
            continue

        rank_k = _rank_order_k_from_values(df, sibling_cols)
        if rank_k is not None:
            reconciled_questions.append(
                _reclassified_question_spec(
                    spec,
                    QuestionType.RANK_ORDER,
                    _RECLASSIFIED_TO_RANK_WARNING,
                    value_range=(1, rank_k),
                )
            )
            changed = True
            continue

        if _looks_like_numeric_allocation(df, sibling_cols):
            reconciled_questions.append(
                _reclassified_question_spec(
                    spec,
                    QuestionType.NUMERIC_ALLOCATION,
                    _RECLASSIFIED_TO_ALLOCATION_WARNING,
                    clear_value_range=True,
                )
            )
            changed = True
            continue
    ...
    if not changed:
        return schema
    return replace(schema, questions=tuple(reconciled_questions))


def _should_reclassify_as_metadata_id(spec: QuestionSpec, df: pd.DataFrame) -> bool:
    ...
    values = df[present_columns[0]].dropna()
    values = values[values.astype(str).str.strip() != ""]
    valid_n = int(len(values))
    if valid_n < 20:
        return False

    distinct = int(values.astype(str).nunique(dropna=True))
    if distinct / valid_n < 0.9:
        return False

    text = f"{spec.canonical_id} {spec.question_text}"
    if _has_id_reference_stem(text):
        return True
    return _values_look_like_metadata_ids(values.astype(str).tolist())


def _value_subtype_sibling_columns(spec: QuestionSpec, df: pd.DataFrame) -> list[str]:
    columns: list[str] = []
    for column in spec.raw_columns:
        if column not in df.columns:
            continue
        label = _subcolumn_label(spec, column)
        if _is_free_text_subcolumn(column, label):
            continue
        columns.append(column)
    return columns


def _subcolumn_label(spec: QuestionSpec, column: str) -> str:
    if column in spec.option_map:
        return str(spec.option_map[column])
    if spec.grid_row_labels and column in spec.grid_row_labels:
        return str(spec.grid_row_labels[column])
    return ""


def _is_free_text_subcolumn(column: str, label: str) -> bool:
    text = f"{column} {label}".casefold()
    return any(token in text for token in _FREE_TEXT_SUBCOLUMN_TOKENS)


def _has_id_reference_stem(text: str) -> bool:
    return _ID_REFERENCE_STEM_PATTERN.search(text) is not None


def _values_look_like_metadata_ids(values: list[str]) -> bool:
    ...
    matches = sum(
        ...
        if any(pattern.fullmatch(value) for pattern in _ID_VALUE_PATTERNS)
    )
    return matches / len(normalized) >= 0.8


def _present_reconciliation_value(value: object) -> bool:
    if pd.isna(value):
        return False
    text = str(value).strip()
    return bool(text) and text.casefold() not in _MISSING_VALUE_TOKENS


def _rank_order_k_from_values(df: pd.DataFrame, sibling_cols: list[str]) -> int | None:
    ranks: list[int] = []
    for column in sibling_cols:
        for value in df[column]:
            if not _present_reconciliation_value(value):
                continue
            rank = _extract_leading_int(value)
            if rank is None:
                return None
            ranks.append(rank)
    ...
    if not ranks:
        return None
    rank_k = max(ranks)
    if rank_k < 2 or rank_k > min(len(sibling_cols), 20):
        return None
    if any(rank < 1 or rank > rank_k for rank in ranks):
        return None
    if not any(rank >= 2 for rank in ranks):
        return None
    if not _has_no_rank_repeats(
        df,
        sibling_cols,
        rank_k,
...
def _looks_like_numeric_allocation(df: pd.DataFrame, sibling_cols: list[str]) -> bool:
    numeric_columns: dict[str, list[float | None]] = {}
    observed_values: list[float] = []
    for column in sibling_cols:
        numeric_column: list[float | None] = []
        for value in df[column]:
            if not _present_reconciliation_value(value):
                numeric_column.append(None)
                continue
            numeric_value = _coerce_to_numeric(value)
            if numeric_value is None:
                return False
            numeric_column.append(float(numeric_value))
            observed_values.append(float(numeric_value))
        numeric_columns[column] = numeric_column

    if not observed_values:
        return False
    if all(value in {0.0, 1.0} for value in observed_values):
        return False

    numeric_df = pd.DataFrame(numeric_columns, index=df.index)
    answered_mask = numeric_df.notna().any(axis=1)
    answered_n = int(answered_mask.sum())
    if answered_n == 0:
        return False
    row_sums = numeric_df.loc[answered_mask].sum(axis=1, skipna=True)
    within_tolerance = (row_sums - 100.0).abs() <= 1.0
    return float(within_tolerance.mean()) >= 0.9


def _reclassified_question_spec(
    spec: QuestionSpec,
    question_type: QuestionType,
    warning: str,
    *,
    value_range: tuple[int, int] | None = None,
    clear_value_range: bool = False,
) -> QuestionSpec:
    ...
    next_value_range = (
        None
        if clear_value_range
        else value_range if value_range is not None else spec.value_range
    )
    return replace(
        spec,
        question_type=question_type,
        value_range=next_value_range,
        warnings=warnings,
    )
...
def _build_question_spec(
    ...
    if not question.get("sub_columns"):
        question = apply_grid_rated_double_colon_matching(question, raw_column_order)
    if not question.get("sub_columns"):
        question = apply_grid_categorical_row_matching(question, raw_column_order)
    ...
    if question_type is QuestionType.METADATA_OR_ID:
        analysis_eligible = False
        exclusion_reason = _METADATA_ID_EXCLUSION_REASON
    ...
    schema_warnings = (
        label_pattern_warnings(question)
        + grid_rated_double_colon_warnings(question)
        + grid_categorical_row_warnings(question)
    )
    if schema_warnings:
    ...
    spec = QuestionSpec(
        ...
        warnings=tuple(schema_warnings),
    )
    return replace(
        spec,
...
def _classify_question(
    ...
    if type_hint == "values_range":
        ...
        if has_sub_columns and has_options:
            # A checkbox multi-select has BOTH per-option sub-columns (Q1r1, Q1r2,
            # ...) AND a tiny 0/1 options table (Unchecked / Checked). That is a
            # MULTI_SELECT_BINARY, not a grid: the real per-option labels live on
            # the sub-columns, and treating it as a grid loses them (every option
            # shows "Checked"/"1"). A genuine grid-single-select has a meaningful
            # option scale (e.g. agree/disagree), not just checked/unchecked.
            option_labels = [label for _code, label in question["options"]]
            if value_range == (0, 1) and _labels_are_binary_select(option_labels):
                return QuestionType.MULTI_SELECT_BINARY
            subtype, _confidence = classify_grid_subtype(question, set())
            if subtype == GRID_RATED and _has_explicit_rating_scale_text(
                question["question_text"]
            ):
                return QuestionType.GRID_RATED
            return QuestionType.GRID_SINGLE_SELECT
        if has_sub_columns and not has_options:
            if _has_double_colon_sub_columns(question):
                return QuestionType.GRID_RATED
...
def _has_double_colon_sub_columns(question: ParsedQuestion) -> bool:
    return any(
        " :: " in str(sub_column_id) or " :: " in str(label)
        for sub_column_id, label in question["sub_columns"]
    )


def _is_nps_question(question: ParsedQuestion) -> bool:
    question_text = question.get("question_text", "")
    if _NPS_RECOMMEND_PATTERN.search(question_text) is None:
        return False
    if _NPS_SCALE_PATTERN.search(question_text) is None:
        return False
    return _has_nps_scale_signal(question)


def _has_nps_scale_signal(question: ParsedQuestion) -> bool:
    value_range = question.get("value_range")
    if value_range is not None:
        low, high = value_range
        if 0 <= low <= 10 and 7 <= high <= 10:
            return True

    numeric_values = [
        value
        for _code, label in question.get("options", [])
        if (value := _coerce_to_numeric(label)) is not None
    ]
    if numeric_values:
        return min(numeric_values) >= 0 and max(numeric_values) <= 10 and max(numeric_values) >= 7
    return False
...
def _is_metadata(
    ...
    if canonical_id in METADATA_CANONICAL_IDS:
        return True
    if _has_id_reference_stem(f"{canonical_id} {question_text}"):
        return True
    if (
...
def _option_map_for_spec(
    ...
    if question_type is QuestionType.NPS:
        return {sub_column_id: label for sub_column_id, label in question["sub_columns"]}
...
def classify_grid_subtype(
    ...
    numeric_ratio = len(numeric_values) / len(labels) if labels else 0.0
    if _is_decisive_numeric_rating_grid(
        value_range,
        question_text,
        numeric_values,
        numeric_ratio,
...
def _is_decisive_numeric_rating_grid(
    value_range: tuple[int, int] | None,
    question_text: str,
    ...
    if value_range is None or value_range == (0, 1):
        return False
    low, high = value_range
    is_numeric_scale = low >= 0 and high <= 10 and (high - low) >= 3
    if not is_numeric_scale:
        return False

    stem = question_text.lower()
    stem_has_explicit_scale = _has_explicit_rating_scale_text(stem)
    stem_has_rating_word = any(token in stem for token in ("rate", "rating", "scale"))
    labels_are_numeric_scale = (
        numeric_ratio >= 0.8 and _numeric_values_form_rating_scale(numeric_values)
    )
    has_grid_cell_columns = any(re.match(r"^.+r\d+c\d+$", column) for column in raw_columns)
    return (
        labels_are_numeric_scale
        or stem_has_explicit_scale
        or (stem_has_rating_word and has_grid_cell_columns)
    )


def _has_explicit_rating_scale_text(text: str) -> bool:
    stem = str(text).lower()
    return any(token in stem for token in ("1-5", "1 to 5", "1-7", "1-10"))
