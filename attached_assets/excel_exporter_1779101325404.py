        ...
        for result in theme_results:
            ...
            helpers.append(
                _StreamingHelperSpec(
                    header=pass_name.removesuffix("_data"),
                    data_name=pass_name,
                    formula_builder=_stream_per_question_match_builder(q_filter_prefix),
                )
            )
            next_col += 1
...
def _stream_per_question_match_builder(q_filter_prefix: str) -> Any:
    return lambda row_index: _raw_custom_filter_match_formula(
        row_index=row_index,
...
def _live_write_cross_tab_table(
    ...
    total_col = first_col + (max_groups * 2) + 1
    ...
    for offset in range(max_groups):
        group_col = first_col + 1 + (offset * 2)
        pct_col = group_col + 1
        ...
        worksheet.cell(row=subheader_row, column=pct_col, value="% of resp").font = _live_font(bold=True)
    ...
    for row_offset, row in enumerate(rows):
        ...
        for offset in range(max_groups):
            ...
            if offset < top_groups:
                ...
                denominator_formula = _build_countifs_formula(
                    "respondent_id_data",
                    "<>",
                    sheet_filters,
                    fq_name,
                ...
                denominator_expression = denominator_formula.lstrip("=")
            else:
                first_group_col = first_col + 1
                last_top_count_col = first_col + 1 + ((top_groups - 1) * 2)
                top_count_refs = ",".join(
                    f"{_openpyxl_column_letter(col)}{excel_row}"
                    for col in range(first_group_col, last_top_count_col + 1, 2)
                )
                top_header_denominators = "+".join(
                    _build_countifs_formula(
                        ...
                        extra_pairs=[
                            (
                                ct_range,
                                f"{_openpyxl_column_letter(col)}${header_row}",
                            )
                        ],
                        theme_prefix=theme_prefix,
                    ).lstrip("=")
                ...
                normal_denominator_formula = _build_countifs_formula(
                    "respondent_id_data",
                    "<>",
                    sheet_filters,
                    fq_name,
                ...
                denominator_expression = (
                    f'IF({header_ref}="Other",'
                    f'{filtered_total_formula.lstrip("=")}-({top_header_denominators}),'
                    f"{normal_denominator_formula})"
                )
            _live_formula(
                worksheet,
                excel_row,
            ...
            count_ref = f"{_openpyxl_column_letter(group_col)}{excel_row}"
            _live_formula(
                worksheet,
                excel_row,
                pct_col,
                f'=IFERROR({count_ref}/({denominator_expression}),0)',
                0,
            ).number_format = "0.0%"
    ...
    if rows:
        ...
        for offset in range(max_groups):
            pct_col = first_col + 2 + (offset * 2)
            _apply_color_scale_range(worksheet, data_start, pct_col, data_end, pct_col)
