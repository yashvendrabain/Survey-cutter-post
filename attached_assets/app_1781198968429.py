...
def _render_journey_rail() -> None:
    ...
    # Clickable navigation under the rail (native buttons -> reliable, no JS).
    nav_targets = [
        ("Upload", "upload"),
        ("Configure", "upload"),
        ("Run & review", "single"),
    ...
    jcols = app.columns(4)
    for ci, (jlabel, jview) in enumerate(nav_targets):
        with jcols[ci]:
            if app.button(jlabel, key=f"journey_jump_{ci}", use_container_width=True):
                _set_current_nav_view(jview)
                app.rerun()
...
def _render_output_structure_gate() -> bool:
    ...
    if app.session_state.get("wiz_output_structure"):
        ...
        _section_header_no_num("CONFIGURE YOUR EXCEL OUTPUT STRUCTURE")
...
def _render_wizard_stepper() -> None:
    ...
    for index, label in enumerate(labels, start=1):
        ...
        with cols[index - 1]:
            app.markdown(
                f"<div class='wiz-step-circle {css_class}'>{index}</div>"
                f"<div class='wiz-step-label' style='font-weight:{weight}'>{html.escape(label)}</div>",
                unsafe_allow_html=True,
            )
            # Clickable: jump straight to this wizard step.
            if app.button(f"Go to {label}", key=f"wizstep_jump_{index}", use_container_width=True):
