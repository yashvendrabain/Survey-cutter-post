...
TOUR_STEPS: list[dict[str, str]] = [
    ...
    {
        "title": "Step 2 — Choose your Excel layout",
    ...
    {
        "title": "Step 3 — Set up the wizard",
        "body": "A short 3-step setup follows: Categories (group/rename questions), "
                "Local Filters (slicers shown atop each sheet), and Cross-cuts "
                "(how cross-cut sheets are laid out). Walk through it, then scroll "
                "back up to Run.",
        "selector": "",
...
(function(){
  ...
  function clear(){ ["sae-tour-overlay","sae-tour-card","sae-tour-spotbox"].forEach(function(id){
    ...
    // restore any element we lifted above the dim
    try {
      var lifted = doc.querySelectorAll('[data-sae-tour-lifted="1"]');
      Array.prototype.forEach.call(lifted, function(el){
        el.style.zIndex = ""; el.removeAttribute("data-sae-tour-lifted");
      });
    } catch(e){}
  }
  function done(){ clear();
  ...
  function render(){
    ...
    // find_text: locate a button whose text contains the phrase (e.g. "Run analysis").
    if (!tgt && step.find_text) {
      var bb = doc.querySelectorAll('button');
      for (var bi=0; bi<bb.length; bi++) {
        if ((bb[bi].textContent||'').trim().toLowerCase().indexOf(step.find_text.toLowerCase()) !== -1) {
          tgt = bb[bi]; break;
        }
      }
    }
    // scroll_text: scroll the matching heading into view (no spotlight needed).
    if (!tgt && step.scroll_text) {
      var all = doc.querySelectorAll('h1,h2,h3,p,div,span,label');
      for (var si=0; si<all.length; si++) {
        if ((all[si].textContent||'').toLowerCase().indexOf(step.scroll_text.toLowerCase()) !== -1) {
          try { all[si].scrollIntoView({behavior:'smooth', block:'center'}); } catch(e){}
          break;
        }
      }
    }
    if(tgt){
      ...
      // Raise the highlighted element ABOVE the dim so it stays clickable.
      try {
        tgt.dataset.saeTourLifted = "1";
        tgt.style.position = tgt.style.position || "relative";
        tgt.style.zIndex = "2000050";
      } catch(e){}
      ...
      // Place the card BELOW the target, or above it if no room.
      var below = r.bottom + 14;
      var cardTop = (below + 230 > window.parent.innerHeight) ? Math.max(12, r.top - 240) : below;
      card.style.top = cardTop + "px";
      card.style.left = Math.min(Math.max(12, r.left), window.parent.innerWidth-360)+"px";
    } else {
