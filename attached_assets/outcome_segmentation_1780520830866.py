    ...
    for question in schema.questions:
        ...
        audit_log.append(
            _make_audit_record(
                question=question,
                segment_definition=segment_definition,
                winner_count=int(round(result.top_option_winner_rate * result.winner_n)),
                winner_n=result.winner_n,
                loser_n=result.loser_n,
...
def _compute_single_select_cohort_results(
    question: QuestionSpec,
    df: pd.DataFrame,
    winner_mask: pd.Series,
    loser_mask: pd.Series,
) -> tuple[SingleSelectResult, SingleSelectResult]:
    return (
        compute_single_select(
            ...
            filter_expr="Winner cohort",
        ),
        compute_single_select(
            question,
            df,
            CalculationLog(),
            filter_mask=loser_mask,
            filter_expr="Laggard cohort",
        ),
    )


def _compute_multi_select_cohort_results(
    question: QuestionSpec,
    df: pd.DataFrame,
    winner_mask: pd.Series,
    loser_mask: pd.Series,
) -> tuple[MultiSelectResult, MultiSelectResult]:
    return (
...
def _single_select_result_count(
    result: SingleSelectResult,
    option_value: Any,
) -> int:
    if option_value in result.distribution:
        return int(result.distribution[option_value].get("count", 0) or 0)
    option_text = str(option_value)
    for candidate_value, payload in result.distribution.items():
        if str(candidate_value) == option_text:
            return int(payload.get("count", 0) or 0)
    return 0


def _multi_select_result_count(
...
def _single_select_differentiator(
    ...
    series = df[column]
    winner_result, loser_result = _compute_single_select_cohort_results(
        question,
        df,
        winner_mask,
        loser_mask,
...
def _multi_select_differentiator(
    ...
    try:
        winner_result, loser_result = _compute_multi_select_cohort_results(
            question,
            df,
            winner_mask,
            loser_mask,
        ...
        raise _SkipQuestion("missing_column") from exc

    winner_valid_n = int(winner_result.respondents_who_answered_any)
    loser_valid_n = int(loser_result.respondents_who_answered_any)
    ...
    for column in question.raw_columns:
        ...
        winner_count = _multi_select_result_count(winner_result, column)
        loser_count = _multi_select_result_count(loser_result, column)
        table = pd.DataFrame(
            [
                [winner_count, loser_count],
                [
                    max(winner_valid_n - winner_count, 0),
                    max(loser_valid_n - loser_count, 0),
                ],
            ],
            index=[1, 0],
        ...
        winner_rate = _safe_rate(winner_count, winner_valid_n)
        loser_rate = _safe_rate(loser_count, loser_valid_n)
        lift, warnings = _lift(winner_rate, loser_rate)
        result = DifferentiatorResult(
            question_id=question.canonical_id,
            question_text=question.question_text,
            question_type="multi_select_binary",
            cramers_v=cramers_v,
            top_option_label=str(label),
            top_option_winner_rate=winner_rate,
            top_option_loser_rate=loser_rate,
...
def _categorical_differentiator(
    ...
    winner_result: SingleSelectResult | None = None,
    loser_result: SingleSelectResult | None = None,
) -> DifferentiatorResult:
    if winner_result is not None and loser_result is not None:
        winner_valid_n = int(winner_result.valid_n)
        loser_valid_n = int(loser_result.valid_n)
    else:
    ...
    if winner_result is not None and loser_result is not None:
        values = sorted(
            set(winner_result.distribution) | set(loser_result.distribution),
    ...
    for value in values:
        if winner_result is not None and loser_result is not None:
            winner_count = _single_select_result_count(winner_result, value)
            loser_count = _single_select_result_count(loser_result, value)
        else:
...
def _compute_laggard_top_option(
    ...
    try:
        if question.question_type is QuestionType.SINGLE_SELECT:
            ...
            loser_denominator = int(loser_result.valid_n)
            if winner_denominator == 0 or loser_denominator == 0:
                return ("", 0.0, 0.0, 0.0)
            ...
            for code, label in question.option_map.items():
                w_count = _single_select_result_count(winner_result, code)
                l_count = _single_select_result_count(loser_result, code)
                w_rate = _safe_rate(w_count, winner_denominator)
                l_rate = _safe_rate(l_count, loser_denominator)
        ...
        if question.question_type is QuestionType.MULTI_SELECT_BINARY:
            ...
            loser_denominator = int(loser_result.respondents_who_answered_any)
            if winner_denominator == 0 or loser_denominator == 0:
                return ("", 0.0, 0.0, 0.0)
            best_label = ""
            best_loser_rate = -1.0
            ...
            for col, label in question.option_map.items():
                if col not in decoded_df.columns:
                    continue
                w_count = _multi_select_result_count(winner_result, col)
                l_count = _multi_select_result_count(loser_result, col)
                w_rate = _safe_rate(w_count, winner_denominator)
                l_rate = _safe_rate(l_count, loser_denominator)
                if l_rate > best_loser_rate:
                    best_loser_rate = l_rate
