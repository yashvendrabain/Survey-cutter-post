...
def _render_chart_type_override_control(
    ...
    selected_type = app.selectbox(
        ...
        index=options.index(selected_type) if selected_type in options else options.index(default_type),
        key=widget_key,
    )
...
def _run_pipeline(
    ...
    schema = _run_with_status_heartbeat(
        status,
    ...
    themes = _run_with_status_heartbeat(
        status,
        start_label="Stage 4/5: Generating themes...",
        heartbeat_labels=(
            "Stage 4/5: Grouping related questions...",
            "Stage 4/5: Reviewing theme assignments...",
            "Stage 4/5: Preparing workbook labels...",
        ),
        work=lambda: categorize_questions_into_themes(
            questions_for_themes,
            cache=_INSIGHT_CACHE,
        ),
        complete_label="Stage 4/5: Themes generated.",
    )
    ...
    demo_priority = _run_with_status_heartbeat(
        status,
        start_label="Stage 4/5: Prioritizing demographics...",
        heartbeat_labels=(
            "Stage 4/5: Reviewing demographic questions...",
            "Stage 4/5: Ranking demographic fields...",
        ),
        work=lambda: categorize_demographic_questions(
            demographic_questions_for_priority,
            cache=_INSIGHT_CACHE,
        ),
        complete_label="Stage 4/5: Demographics prioritized.",
    )
    short_labels = _run_with_status_heartbeat(
        status,
        start_label="Stage 4/5: Generating short labels...",
        heartbeat_labels=(
            "Stage 4/5: Shortening question labels...",
            "Stage 4/5: Checking label readability...",
        ),
        work=lambda: generate_short_labels(
            [
        ...
        complete_label="Stage 4/5: Labels generated.",
    )
...
def _render_single_cut_result(result: Any, spec: Any) -> None:
    ...
    elif isinstance(result, (RankOrderResult, GridRatedResult, GridBinaryPivotResult)):
        _render_recommended_chart_and_thinkcell(result, spec)
    else:
...
def _probe_format_wizard(uploaded_files: list[Any]) -> dict[str, Any]:
    ...
    try:
        ...
        needs_wizard, scores = get_default_registry().needs_wizard(workbook, scores_raw_df)
...
def _section_survey_classification() -> None:
    ...
    if app.session_state.get("seg_mode_radio") == "manual_uuid" and manual_cohort is not None:
        app.success(
            "Manual cohort detected: "
            f"{len(manual_cohort.winner_uuids)} winners, "
            f"{len(manual_cohort.laggard_uuids)} laggards from uploaded file"
        )
        app.caption("Manual UUID mode hides the outcome variable picker.")
...
def _render_segment_definition_ui() -> None:
    ...
    _SEG_MODE_LABELS = {
        "manual_uuid": "Manual UUID upload",
    ...
    if seg_mode != "manual_uuid":
        override_laggard_outcome = app.checkbox(
            "Override outcome question for laggards",
            value=False,
    ...
    if seg_mode == "manual_uuid":
        ...
        if manual_cohort is not None:
            ...
            app.write(
                f"Winners: {len(manual_cohort.winner_uuids)} respondents "
                f"({valid_winners} valid, {len(manual_cohort.winner_uuids) - valid_winners} not found in raw data)"
            )
            app.write(
                f"Laggards: {len(manual_cohort.laggard_uuids)} respondents "
            ...
            app.write(f"Overlap (in both winner and laggard lists): {len(overlap)} respondents")
            if invalid_n:
        ...
        if not options:
            ...
            laggard_labels_selected = app.multiselect(
                "Laggard option values",
                options=option_labels,
                help=(
                    "Respondents matching these codes are the laggard cohort. "
                    "Leave blank to use all non-winner valid responses."
                ),
            ...
            if winner_codes:
                ...
                segment_definition = SegmentDefinition(
                    ...
                    laggard_label=loser_label or "Laggards",
        ...
        segment_definition = SegmentDefinition(
            ...
            laggard_label=laggard_label or "Laggards",
            loser_label=laggard_label or "Laggard",
            laggard_outcome_question_id=laggard_outcome_question_id,
            laggard_outcome_sub_question_id=laggard_outcome_sub_question_id,
        )
...
def _active_filters_summary() -> str:
    ...
    gf_state = app.session_state.get("global_filter_state")
    if gf_state is not None and gf_state.is_active():
        return gf_state.description()
    return "(all respondents)"


def _hypothesis_cross_cut_lookup() -> dict[Any, str]:
    app = _require_streamlit()
    lookup: dict[Any, str] = {}
    for result in app.session_state.get("cross_cut_results", []):
        ids = tuple(getattr(result, "source_question_ids", ()) or ())
        if len(ids) == 2:
            lookup[(ids[0], ids[1])] = result.cross_cut_id
            lookup[(ids[1], ids[0])] = result.cross_cut_id
    return lookup


def _run_hypothesis_spec(spec: Any) -> None:
    ...
    result = validate_hypothesis(
        ...
        cross_cut_lookup=_hypothesis_cross_cut_lookup(),
        active_filters_summary=_active_filters_summary(),
    )
    app.session_state.setdefault("hypothesis_results", []).append(result)
    _refresh_full_workbook()


def _section_hypothesis_check() -> None:
    app = _require_streamlit()
    app.markdown('<div id="section-hypothesis"></div>', unsafe_allow_html=True)
    _section_header("5", "Hypothesis Check", anchor="section-5", meta="Statistical validation")

    if not app.session_state["run_complete"]:
        app.caption("Run an analysis first to validate hypotheses.")
        return

    from src.hypothesis_validator import parse_freetext_hypothesis
    from src.models import HypothesisGroup, HypothesisSpec, QuestionType

    schema = app.session_state["schema"]
    eligible = [q for q in schema.questions if q.analysis_eligible]
    question_options = [
        (f"{q.canonical_id}: {(q.question_text or '')[:70]}", q.canonical_id)
        for q in eligible
    ]
    if len(question_options) < 2:
        app.info("At least two eligible questions are needed for a hypothesis check.")
        return

    with app.form("hyp_structured_form"):
        app.subheader("Structured hypothesis")
        title = app.text_input("Title", key="hyp_title")
        outcome_pick = app.selectbox(
            "Outcome question",
            options=question_options,
            format_func=lambda item: item[0],
            key="hyp_outcome_q",
        )
        predictor_pick = app.selectbox(
            "Predictor question",
            options=question_options,
            format_func=lambda item: item[0],
            key="hyp_predictor_q",
        )
        direction = app.selectbox(
        ...
        predictor_spec = schema.get_question(predictor_pick[1])
        predictor_groups: list[HypothesisGroup] = []
        if predictor_spec is not None and predictor_spec.question_type in {
            QuestionType.SINGLE_SELECT,
            QuestionType.MULTI_SELECT_BINARY,
        }:
            app.caption("Optional categorical grouping")
            option_items = list((predictor_spec.option_map or {}).items())
            if not option_items and predictor_spec.question_type is QuestionType.MULTI_SELECT_BINARY:
                option_items = [("Selected", "Selected"), ("Not selected", "Not selected")]
            group_cols = app.columns(2)
            for idx, col in enumerate(group_cols, start=1):
                with col:
                    group_label = app.text_input(
                        f"Group {idx} label",
                        value=f"Group {idx}",
                        key=f"hyp_group_{idx}_label",
                    )
                    selected = app.multiselect(
                        f"Group {idx} values",
                        options=option_items,
                        format_func=lambda item: f"{item[0]}: {item[1]}",
                        key=f"hyp_group_{idx}_values",
                    )
                    if selected:
                        predictor_groups.append(
                            HypothesisGroup(
                                label=group_label,
                                matching_values=tuple(value for value, _label in selected),
                            )
                        )
        submitted = app.form_submit_button("Run hypothesis", type="primary")
        if submitted:
            spec = HypothesisSpec(
                title=title or "Untitled hypothesis",
                outcome_question_id=outcome_pick[1],
                predictor_question_id=predictor_pick[1],
                outcome_direction=direction,
                predictor_groups=tuple(predictor_groups),
            )
            _run_hypothesis_spec(spec)
            app.success("Hypothesis check complete.")

    if os.environ.get("PORTKEY_API_KEY", "").strip():
        with app.expander("Parse from plain English"):
            text = app.text_area(
                "Type your hypothesis in plain English",
                key="hyp_free_text",
            )
            if app.button("Parse and run", key="hyp_parse_run_button"):
                parsed = parse_freetext_hypothesis(text, schema)
                if parsed is None:
                    app.error("Could not parse that hypothesis. Use the structured form.")
                else:
                    app.session_state["pending_hypothesis_spec"] = parsed
                    app.json(
                        {
                            "title": parsed.title,
                            "outcome_question_id": parsed.outcome_question_id,
                            "predictor_question_id": parsed.predictor_question_id,
                            "outcome_direction": parsed.outcome_direction,
                        }
                    )
            pending = app.session_state.get("pending_hypothesis_spec")
            if pending is not None and app.button("Confirm parsed hypothesis", key="hyp_confirm_parsed"):
                _run_hypothesis_spec(pending)
                app.session_state["pending_hypothesis_spec"] = None
                app.success("Hypothesis check complete.")

    results = app.session_state.get("hypothesis_results", [])
    if not results:
        app.caption("No hypotheses run yet.")
        return
    ...
    for index, result in enumerate(results, start=1):
        color = {
            ...
            "REFUTED": "#CC0000",
            "INCONCLUSIVE": "#666666",
        }.get(result.verdict, "#666666")
        statistic = result.statistic
        app.markdown(
            f"<div style='border:1px solid #E5E5E5; padding:16px; margin:12px 0;'>"
            f"<div style='font-weight:700;'>{result.spec.title}</div>"
            f"<div style='color:{color}; font-weight:700;'>{result.verdict}</div>"
            f"<div>Test: {statistic.test_name if statistic else 'n/a'} | "
            f"Effect: {statistic.effect_size if statistic else 'n/a'} | "
            f"p-value: {statistic.p_value if statistic else 'n/a'} | "
            f"n={result.pairwise_n}</div></div>",
            unsafe_allow_html=True,
        )
        with app.expander(f"Show calculation details #{index}"):
            app.write("Calculation_Log entries")
            app.code("\n".join(result.calculation_log_entry_ids))
            if result.related_cross_cut_id:
                app.write(f"Related cross-cut: {result.related_cross_cut_id}")
            else:
                app.caption("No matching cross-cut found.")
        with app.expander(f"Why this verdict? #{index}"):
            app.write(result.verdict_reason)
            if result.ai_prose_explanation:
                app.write(result.ai_prose_explanation)
...
def _section_downloads() -> None:
    ...
    _section_header("6", SECTION_DOWNLOADS, anchor="section-6", meta="Workbooks")
    ...
    with col_outcome:
        ...
        if app.session_state.get("outcome_segmented_workbook_bytes"):
            app.download_button(
                label="Outcome Segmented Workbook (Winners vs Laggards)",
...
def _render_nav_bar() -> None:
    ...
    n_hypotheses = len(app.session_state.get("hypothesis_results", []))
    seg = app.session_state.get("segmentation_result")
    ...
    tabs_meta = [
        ...
        ("section-hypothesis", "Hypothesis",  str(n_hypotheses) if n_hypotheses else None, None),
