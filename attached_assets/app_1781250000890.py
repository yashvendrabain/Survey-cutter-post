...
def _render_single_cut_card(
    ...
    with app.expander(expander_label, expanded=expanded):
        ...
        if filtered is not None:
            ...
            with _fcol2:
                app.markdown(
                    "<style>.rm-filter-btn + div .stButton button{"
                    "white-space:nowrap !important;min-width:max-content !important;}"
                    "</style><div class='rm-filter-btn'></div>",
                    unsafe_allow_html=True,
                )
                if app.button(
                    "Remove filter",
                    key=f"{filter_key}_remove",
                    type="tertiary",
                    help="Clear this question's filter",
                ):
    ...
    # the API error detail when the AI call fails. All table kinds (single cut
    # AND cross-cut breakdowns) now use the manual "Generate insight" button so
    # the cross cut has an explicit button too (no surprise auto API calls).
...
def _render_selected_question_cross_cut_builder(target_id: str) -> None:
    ...
    # Build the dimension list from canonical questions (one entry per question),
    # NOT from option-expanded cross_cut_question_options — otherwise a
    # multi-select like Q15 shows up once per option ("Q15: Marketing",
    # "Q15: Customer experience", ...) instead of a single "Q15" dimension.
    dimension_options: list[str] = []
    seen_dimension_ids: set[str] = set()
    for spec in schema.questions:
        canonical = getattr(spec, "canonical_id", None)
        if not canonical or canonical == target_id or canonical in seen_dimension_ids:
            continue
        if spec.question_type in dimension_types and getattr(
            spec, "analysis_eligible", True
        ):
            dimension_options.append(canonical)
            seen_dimension_ids.add(canonical)
...
def _render_wizard_step_filters() -> None:
    ...
    with _lf_left:
        ...
        app.markdown(
            "<div style='font-size:13px;color:#444;line-height:1.7;min-height:120px;padding-top:4px;'>"
