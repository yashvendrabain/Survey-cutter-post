...
def _build_filters_sheet(
    ...
    for question in demographic_questions[:8]:
        ...
        wrapped_cell = worksheet.cell(
            row=row_index,
            column=3,
            value=_wrapped_formula(filter_name),
        )
...
def _build_theme_local_filters_block(
    ...
    for sheet_filter in workbook_filters:
        ...
        resolved_cell = worksheet.cell(
            row=row_index,
            column=3,
            value=f'=IF({local_name}="(Inherit)",{global_name},{local_name})',
        )
        wrapped_cell = worksheet.cell(row=row_index, column=4, value=_wrapped_formula(resolved_name))
...
def _write_theme_custom_filter(
    ...
    q_resolved_cell = worksheet.cell(
        row=row_index,
        column=3,
        value=f'=IF({local_q}="(Inherit)",{global_question_name},{local_q})',
    )
    ...
    v_resolved_cell = worksheet.cell(
        row=row_index,
        column=3,
        value=f'=IF({local_v}="(Inherit)",{global_value_name},{local_v})',
    )
    wrapped_cell = worksheet.cell(row=row_index, column=4, value=_wrapped_formula(resolved_v))
    worksheet.cell(row=row_index, column=5, value="Choose one value or type comma-separated values")
    resolved_column_cell = worksheet.cell(
        row=row_index,
        column=6,
        value=f'=IF({resolved_q}="(None)","respondent_id_data",{resolved_q} & "_data")',
    )
...
def _live_write_custom_filter_slot(
    ...
    wrapped_cell = worksheet.cell(
        row=row_index,
        column=col_index + 5,
        value=_wrapped_formula(value_name),
    )
...
def _live_write_question_block(
    ...
    wrapped_cell = worksheet.cell(row=row_index, column=7, value=_wrapped_formula(fv_name))
    ...
    if result.question_type is QuestionType.NUMERIC_ALLOCATION:
        for option_id, payload in (result.per_option_stats or {}).items():
            ...
            for metric in ("Mean", "Min", "Max"):
                ...
                worksheet.cell(row=row_index, column=4, value=_build_numeric_count_formula(column.data_name, sheet_filters, fq_name, fv_name, theme_prefix))
                row_index += 1
    ...
    for row_offset, row in enumerate(rows):
        ...
        for offset in range(8):
            ...
            worksheet.cell(
                row=excel_row,
                column=col_index,
                value=f'=({ct_name}<>"(None)")*{countifs_formula}',
            )
        worksheet.cell(
            row=excel_row,
            column=first_col + 9,
...
def _build_countifs_formula(
    ...
    del fv_name
    factors = _filter_formula_pairs(sheet_filters)
    if theme_prefix:
        factors.append(
            _dynamic_filter_factor(
                f"{theme_prefix}_F_Custom1",
                f"{theme_prefix}_F_Custom1_Q_resolved",
            )
        )
        factors.append(
            _dynamic_filter_factor(
                f"{theme_prefix}_F_Custom2",
                f"{theme_prefix}_F_Custom2_Q_resolved",
            )
        )
    else:
    ...
    factors.append(_match_factor(data_name, criteria))
    for range_expr, criterion_expr in extra_pairs or []:
        factors.append(_match_factor(range_expr, criterion_expr, criterion_is_expression=True))
    return "=SUMPRODUCT(" + "*".join(factors) + ")"
...
def _build_numeric_count_formula(
    ...
    del fv_name
    return f"=SUMPRODUCT({_numeric_mask_expression(data_name, sheet_filters, fq_name, theme_prefix)})"


def _filter_formula_pairs(sheet_filters: list[dict[str, str]]) -> list[str]:
    return [
        _static_filter_factor(
            sheet_filter["data_name"],
            sheet_filter["value_name"],
            sheet_filter["wrapped_name"],
        )
        for sheet_filter in sheet_filters
    ]


def _numeric_mask_expression(
    data_name: str,
    sheet_filters: list[dict[str, str]],
    fq_name: str,
    theme_prefix: str,
    ...
    factors = _filter_formula_pairs(sheet_filters)
    if theme_prefix:
        factors.append(_dynamic_filter_factor(f"{theme_prefix}_F_Custom1", f"{theme_prefix}_F_Custom1_Q_resolved"))
        factors.append(_dynamic_filter_factor(f"{theme_prefix}_F_Custom2", f"{theme_prefix}_F_Custom2_Q_resolved"))
    else:
        factors.append(_dynamic_filter_factor("F_Custom1", "F_Custom1_Q"))
        factors.append(_dynamic_filter_factor("F_Custom2", "F_Custom2_Q"))
    factors.append(_dynamic_filter_factor(fq_name[:-2], fq_name))
    factors.append(f"ISNUMBER({data_name})*1")
    return "*".join(factors)
...
def _static_filter_factor(data_name: str, value_name: str, wrapped_name: str) -> str:
    return (
        ...
        f'ISNUMBER(SEARCH("|"&{data_name}&"|",{wrapped_name})))>0)*1'
    )


def _dynamic_filter_factor(prefix: str, question_name: str) -> str:
    return (
        f'((({question_name}="(None)")+({prefix}_wrapped="|(All)|")+'
        f'ISNUMBER(SEARCH("|"&INDIRECT({prefix}_resolved_column)&"|",{prefix}_wrapped)))>0)*1'
    )


def _match_factor(data_name: str, criteria: Any, criterion_is_expression: bool = False) -> str:
    if criterion_is_expression:
        return f"({data_name}={criteria})*1"
    if criteria == "<>":
        return f'({data_name}<>"")*1'
    return f"({data_name}={_excel_criteria(criteria)})*1"
