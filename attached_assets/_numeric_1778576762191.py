...
def _allocation_per_option_stats(
    ...
    for column_id, series in columns.items():
        ...
        stats[column_id] = {
            "mean": round(float(valid_values.mean()), 1) if valid_n else 0.0,
            "median": round(float(valid_values.median()), 1) if valid_n else 0.0,
            "std": round(float(valid_values.std()), 1) if valid_n > 1 else 0.0,
            "min_val": round(float(valid_values.min()), 1) if valid_n else 0.0,
            "max_val": round(float(valid_values.max()), 1) if valid_n else 0.0,
            "valid_n": valid_n,
            "missing_n": missing_n,
        }
    return stats
...
def _direct_numeric_stat_audits(
    ...
    stat_specs = (
        (
            ...
            round(float(summary["std"]), 1) if valid_n > 1 else 0.0,
        ),
        (
            "numeric_p25",
            "p25 = series.dropna().quantile(0.25)",
            round(float(summary["p25"]), 1) if valid_n else 0.0,
        ),
        (
            "numeric_p50",
            "p50 = series.dropna().quantile(0.50)",
            round(float(summary["p50"]), 1) if valid_n else 0.0,
        ),
        (
            "numeric_p75",
            "p75 = series.dropna().quantile(0.75)",
            round(float(summary["p75"]), 1) if valid_n else 0.0,
        ),
    )
    return tuple(
        AuditRecord(
