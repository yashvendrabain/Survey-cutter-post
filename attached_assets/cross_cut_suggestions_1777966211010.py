...
def _segment_profile_candidates(schema: SurveySchema) -> list[Candidate]:
    ...
    for demographic in demographics:
        ...
        for option_value, option_label in _sorted_options(demographic):
...
def _is_demographic(question: QuestionSpec) -> bool:
    if question.question_type is not QuestionType.SINGLE_SELECT:
        return False
    if not 2 <= len(question.option_map) <= 8:
        return False
