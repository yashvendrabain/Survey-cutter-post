...
class WizardFilterConfig:
    workbook_custom_filter_count: int = CUSTOM_FILTER_DEFAULT
    per_question_filter_count: int = PER_QUESTION_FILTER_DEFAULT
...
def filter_config_from_wizard_state(state: dict[str, Any]) -> WizardFilterConfig:
    """Return exporter-ready filter slot counts from wizard session state."""

    return WizardFilterConfig(
        workbook_custom_filter_count=normalise_custom_filter_count(
            state.get("wizard_workbook_custom_filter_count", state.get("wiz_num_custom_filters"))
        ),
        per_question_filter_count=normalise_per_question_filter_count(
            state.get("wizard_per_question_filter_count", state.get("wiz_num_per_question_filters"))
        ),
    )
