...
def compute_rank_order(
    ...
    for column in columns:
        ...
        for rank, count in enumerate(counts_per_rank, start=1):
            ...
            log.record(audit)
        net_rank_score, points_sum = _net_rank_score(
            counts_per_rank,
            rank_k,
            total_respondents,
        )
        log.record(
            _make_audit_record(
                metric_name="net_rank_score",
                question_id=question_spec.canonical_id,
                source_columns=(column,),
                filter_expr=filter_expr,
                numerator=points_sum,
                denominator=rank_k * total_respondents,
                formula="sum((K - rank + 1) * count_at_rank) / (K * ranking_respondents) * 100",
                value_raw=net_rank_score,
                valid_n=total_respondents,
                missing_n=missing_n,
                output_sheet=f"SC_{question_spec.canonical_id}",
            )
...
def _net_rank_score(
    ...
    denominator = rank_k * total_respondents
    score = (points_sum / denominator * 100.0) if denominator else 0.0
    return float(score), int(points_sum)
