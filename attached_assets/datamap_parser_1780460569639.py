...
def parse_datamap(
    ...
    *,
    min_questions: int = 2,
    datamap_sheet_hint: str | None = None,
) -> DataMap:
    """Parse a data map workbook using the adapter registry."""

    workbook = load_workbook(path, read_only=True, data_only=True)
    try:
        if datamap_sheet_hint is not None:
            if datamap_sheet_hint not in workbook.sheetnames:
                available = ", ".join(workbook.sheetnames)
                raise ValueError(
                    f"data map sheet hint {datamap_sheet_hint!r} not found. "
                    f"Available sheets: {available}"
                )
            parser_workbook: Any = _WorkbookWithDatamapSheetHint(
                workbook,
                datamap_sheet_hint,
            )
        elif DATAMAP_SHEET_NAME not in workbook.sheetnames:
            available = ", ".join(workbook.sheetnames)
            raise ValueError(
...
class _WorkbookWithDatamapSheetHint:
    """Expose an in-memory Sheet1 alias for an already-identified data map sheet."""

    def __init__(self, workbook: Any, datamap_sheet_name: str) -> None:
        self._workbook = workbook
        self._datamap_sheet_name = datamap_sheet_name

    @property
    def sheetnames(self) -> list[str]:
        names = list(getattr(self._workbook, "sheetnames", []))
        if DATAMAP_SHEET_NAME not in names:
            names.append(DATAMAP_SHEET_NAME)
        return names

    def __getitem__(self, sheet_name: str) -> Any:
        if sheet_name == DATAMAP_SHEET_NAME:
            return self._workbook[self._datamap_sheet_name]
