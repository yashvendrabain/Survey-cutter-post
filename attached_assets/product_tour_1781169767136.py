...
(function(){
  ...
  if (pdoc && pdoc.head && !pdoc.getElementById("sae-tour-style")) {
    ...
    sty.textContent =
      "#sae-tour-card{position:fixed;left:50%;bottom:28px;transform:translateX(-50%);"+
      "width:460px;max-width:92vw;background:#fff;border-radius:14px;"+
      "box-shadow:0 18px 50px rgba(0,0,0,0.28);padding:18px 22px;font-family:Arial,sans-serif;"+
...
(function(){
  ...
  function drawArrow(tgt){
    ...
    if (hasTarget){
      ...
      if (cy < vh*0.6){
        arrow.textContent = "\u2B07";  // big down-arrow above the target
        arrow.style.left = Math.min(Math.max(8, cx-26), vw-60)+"px";
        arrow.style.top  = Math.max(8, r.top-58)+"px";
      } else {
        arrow.textContent = "\u2B06";  // big up-arrow below the target
        arrow.style.left = Math.min(Math.max(8, cx-26), vw-60)+"px";
        arrow.style.top  = Math.min(vh-66, r.bottom+10)+"px";
      }
    } else {
      // Fallback: no locatable target (e.g. an info step) -> point down at the
      // page content from just above the bottom-center card, so EVERY step
      // always shows an arrow.
      arrow.textContent = "\u2B07";
      arrow.style.left = (vw/2 - 26)+"px";
      arrow.style.top  = Math.max(8, vh - 150)+"px";
    }
