...
def _detect_grid_pattern_for_decoding(
    ...
    if _has_explicit_grid_row_sub_columns(question):
        return _GRID_CATEGORICAL_ROW
    return None
...
def _has_categorical_grid_row_columns(
    ...
    if not 2 <= len(options) <= 10:
        return False
    canonical_id = question["canonical_id"]
    if _has_bcn_style_sub_column(canonical_id, raw_columns):
        return False

    row_labels = _infer_grid_row_labels(canonical_id, raw_columns)
    if len(row_labels) < 2:
        return False
    return not any(_label_matches_option(label, options) for label in row_labels)


def _has_explicit_grid_row_sub_columns(question: ParsedQuestion) -> bool:
    options = question.get("options", []) or []
    sub_columns = question.get("sub_columns", []) or []
    if not 2 <= len(options) <= 10 or len(sub_columns) < 2:
        return False
    if _grid_options_look_binary_selection(options):
        return False
    labels = [str(label).strip() for _column, label in sub_columns if str(label).strip()]
    if len(labels) < 2:
        return False
    return not any(_label_matches_option(label, options) for label in labels)
