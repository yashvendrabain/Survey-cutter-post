...
def compute_rank_order(
    ...
    for column in columns:
        ...
        for rank, count in enumerate(counts_per_rank, start=1):
            ...
            log.record(audit)
        weighted_average, points_sum = _weighted_average(
            counts_per_rank,
            rank_k,
            total_respondents,
        )
        log.record(
            _make_audit_record(
                ...
                denominator=total_respondents,
                formula="sum((K - rank + 1) * count_at_rank) / ranking_respondents",
                value_raw=weighted_average,
                valid_n=total_respondents,
                missing_n=missing_n,
                output_sheet=f"SC_{question_spec.canonical_id}",
            )
...
def _weighted_average(
    ...
    score = (points_sum / total_respondents) if total_respondents else 0.0
