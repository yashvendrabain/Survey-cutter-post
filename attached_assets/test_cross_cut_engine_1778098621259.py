...
def grid_single_select_dimension_spec() -> QuestionSpec:
    return QuestionSpec(
        ...
        raw_columns=("Q_GRID_SEGr1",),
        option_map={1: "Segment 1", 2: "Segment 2", 3: "Segment 3"},
        value_range=(1, 3),
        grid_row_labels={"Q_GRID_SEGr1": "Primary segment row"},
    )
...
class TestCrossCutEngine(unittest.TestCase):
    ...
    def test_cross_tab_accepts_grid_single_select_dimension(self) -> None:
        dataframe = load_cross_cut_golden()
        dataframe["Q_GRID_SEG"] = dataframe["Q_SEG_1"]
        dataframe["Q_GRID_SEGr1"] = 9
        log = CalculationLog()
        spec = CrossCutSpec(
        ...
        self.assertEqual(skips, [])
        self.assertEqual(results[0].result_table["counts"], EXPECTED_CROSS_TAB_COUNTS)
        self.assertEqual(log.all_records()[0].source_columns, ("Q_GRID_SEG", "Q_TGT_1"))

    def test_grid_single_select_uses_parent_column(self) -> None:
        dataframe = load_cross_cut_golden()
        dataframe["Q_GRID_SEG"] = dataframe["Q_SEG_1"]
        dataframe["Q_GRID_SEGr1"] = 9
        log = CalculationLog()
        spec = CrossCutSpec(
            cross_cut_id="CC_GRID_PARENT",
            title="Grid parent column by target",
            analysis_type=AnalysisType.CROSS_TAB,
            source_question_ids=("Q_GRID_SEG", "Q_TGT_1"),
        )
        ...
        self.assertEqual(skips, [])
        self.assertEqual(results[0].result_table["row_totals"], EXPECTED_CROSS_TAB_ROW_TOTALS)
        self.assertNotIn(9, results[0].result_table["counts"])
    ...
    def test_group_comparison_accepts_grid_single_select_segment(self) -> None:
        dataframe = load_cross_cut_golden()
        dataframe["Q_GRID_SEG"] = dataframe["Q_SEG_1"]
        dataframe["Q_GRID_SEGr1"] = 9
        log = CalculationLog()
