...
def industry_grid_question() -> QuestionSpec:
    return QuestionSpec(
        question_id="[Q_IND_GRID]",
        canonical_id="Q_IND_GRID",
        question_text="Which industry best describes your company?",
        question_type=QuestionType.GRID_SINGLE_SELECT,
        raw_columns=("Q_IND_GRIDr1", "Q_IND_GRIDr2", "Q_IND_GRIDr3"),
        option_map={1: "Selected"},
        value_range=(0, 1),
        grid_row_labels={
            "Q_IND_GRIDr1": "Tech",
            "Q_IND_GRIDr2": "Retail",
            "Q_IND_GRIDr3": "Healthcare",
        },
    )


def revenue_growth_bucket_question() -> QuestionSpec:
    return q(
        "Q_REVENUE_BUCKET",
        "Which company segment revenue growth bucket applies?",
        QuestionType.SINGLE_SELECT,
        {1: "Decline", 2: "Flat", 3: "Moderate growth", 4: "High growth"},
    )
...
class TestCrossCutSuggestions(unittest.TestCase):
    ...
    def test_cross_tab_suggested_for_demographic_grid_pair(self) -> None:
        suggestions = suggest_cross_cuts(
            schema(industry_grid_question(), revenue_growth_bucket_question())
        )
        ...
        self.assertEqual(len(cross_tabs), 1)
        self.assertEqual(
            cross_tabs[0].source_question_ids,
            ("Q_IND_GRID", "Q_REVENUE_BUCKET"),
        )
