...
def _render_wizard_top_nav() -> None:
    """Clean centered 3-step stepper (Categories / Filter counts / Cross-cut).

    The visual (circles + connectors + labels) is rendered as ONE HTML flexbox
    block so it always aligns; small buttons beneath it handle clicks.
    """
    app = _require_streamlit()
    step = int(app.session_state.get("wiz_step", 1))
    ...
    app.markdown(
        "<style>"
        ".wizstep-ctrls + div [data-testid='stHorizontalBlock']{"
        "max-width:560px;margin:4px auto 2px;gap:0 !important;align-items:center;}"
        # arrow buttons: borderless glyph, centred with circles
        ".wizstep-arrow .stButton button{background:transparent !important;border:none !important;"
        "color:#CC0000 !important;font-size:24px !important;font-weight:700 !important;"
        "box-shadow:none !important;padding:0 !important;line-height:1 !important;min-height:36px !important;}"
        ".wizstep-arrow .stButton button:disabled{color:#D8DADE !important;}"
        # circle buttons: round, sized, centred
        ".wizstep-circle .stButton button{width:36px !important;height:36px !important;"
        "min-height:36px !important;border-radius:50% !important;font-size:14px !important;"
        "font-weight:700 !important;padding:0 !important;margin:0 auto !important;"
        "display:flex !important;align-items:center !important;justify-content:center !important;}"
        ".wizstep-circle .stButton button[kind='primary']{"
        "background:linear-gradient(180deg,#E00000,#CC0000) !important;color:#fff !important;"
        "border:none !important;box-shadow:0 2px 6px rgba(204,0,0,0.25) !important;}"
        ".wizstep-circle .stButton button[kind='secondary']{"
        "background:#F2F3F5 !important;color:#A8ADB3 !important;border:1.5px solid #E2E4E8 !important;"
        "box-shadow:none !important;}"
        "</style><div class='wizstep-ctrls'></div>",
        unsafe_allow_html=True,
    )
    bcol, s1, s2, s3, ncol = app.columns([0.8, 1, 1, 1, 0.8])
    with bcol:
        app.markdown("<div class='wizstep-arrow'></div>", unsafe_allow_html=True)
        if app.button("\u2039", disabled=(step <= 1), key="wiz_back_top", help="Back"):
            app.session_state["wiz_step"] = max(1, step - 1)
            _wizard_rerun(app)
    for idx, scol in enumerate([s1, s2, s3], start=1):
        with scol:
            app.markdown("<div class='wizstep-circle'></div>", unsafe_allow_html=True)
            if app.button(str(idx), key=f"wiztop_circle_{idx}",
                          type=("primary" if idx == step else "secondary")):
                app.session_state["wiz_step"] = idx
                _wizard_rerun(app)
            lab_color = "#CC0000" if idx == step else "#999"
            lab_weight = "700" if idx == step else "500"
            app.markdown(
                f"<div style='text-align:center;font-size:11px;font-weight:{lab_weight};"
                f"color:{lab_color};font-family:Arial,sans-serif;margin-top:3px;"
                f"white-space:nowrap;'>{html.escape(labels[idx-1])}</div>",
                unsafe_allow_html=True,
            )
    with ncol:
        app.markdown("<div class='wizstep-arrow'></div>", unsafe_allow_html=True)
        if app.button("\u203a", disabled=(step >= 3), key="wiz_next_top", help="Next"):
