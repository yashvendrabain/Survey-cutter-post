...
def _wvl_write_single_select_layout(
    ...
    headers = ["Option ID", "Option", f"{winner_label} count", f"{winner_label} %", f"{laggard_label} count", f"{laggard_label} %", "Delta %"]
    ...
    winner_total_row = start_row + len(result.distribution) + 2
    ...
    if data_end >= data_start:
        ...
        _apply_color_scale_range(worksheet, data_start, 6, data_end, 6)
    _wvl_write_totals(worksheet, winner_total_row, laggard_total_row, data_name, fq_name, result.valid_n, theme_prefix)
    return laggard_total_row + 2
...
def _wvl_write_multi_select_layout(
    ...
    headers = ["Option ID", "Option", f"{winner_label} count", f"{winner_label} %", f"{laggard_label} count", f"{laggard_label} %", "Delta %"]
    _live_header_row(worksheet, start_row, headers)
    winner_total_row = start_row + len(result.selections) + 2
    laggard_total_row = winner_total_row + 1
    row_index = start_row + 1
    anchor_data_name = ""
    ...
    if anchor_data_name:
        ...
        if data_end >= data_start:
            _apply_color_scale_range(worksheet, data_start, 4, data_end, 4)
            _apply_color_scale_range(worksheet, data_start, 6, data_end, 6)
        _wvl_write_totals(worksheet, winner_total_row, laggard_total_row, anchor_data_name, fq_name, result.valid_n, theme_prefix)
    return laggard_total_row + 2
...
def _wvl_write_grid_rated_layout(
    ...
    headers = ["Sub-question ID", "Sub-question", f"{winner_label} mean", f"{laggard_label} mean", "Delta mean", f"{winner_label} n", f"{laggard_label} n"]
    _live_header_row(worksheet, start_row, headers)
    row_index = start_row + 1
    for result_row in result.rows:
        ...
        winner_n_cell = _live_formula(worksheet, row_index, 6, _wvl_countifs(column.data_name, "<>", fq_name, "winners_mask_data", theme_prefix), result_row.valid_n_per_column[0] if result_row.valid_n_per_column else 0)
        winner_n_cell.number_format = "#,##0"
        laggard_n_cell = _live_formula(worksheet, row_index, 7, _wvl_countifs(column.data_name, "<>", fq_name, "laggards_mask_data", theme_prefix), result_row.valid_n_per_column[1] if len(result_row.valid_n_per_column) > 1 else 0)
