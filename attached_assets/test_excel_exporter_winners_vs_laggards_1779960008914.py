...
class TestWinnersVsLaggardsWorkbook(unittest.TestCase):
    ...
    def test_single_select_sheet_has_documented_columns(self) -> None:
        ...
        headers = [sheet.cell(row=8, column=col).value for col in range(1, 8)]
    ...
    def test_count_percentage_and_delta_cells_are_formulas(self) -> None:
        _path, workbook, _log = _build_workbook()
        sheet = _data_sheet(workbook)
        self.assertTrue(str(sheet["C9"].value).startswith("="))
        self.assertTrue(str(sheet["D9"].value).startswith("=IFERROR("))
        self.assertTrue(str(sheet["G9"].value).startswith("="))
        self.assertIn("winners_mask_data", sheet["C9"].value)
        self.assertIn("laggards_mask_data", sheet["E9"].value)
    ...
    def test_category_sheet_has_red_banner(self) -> None:
        _path, workbook, _log = _build_workbook()
        sheet = _data_sheet(workbook)
        self.assertEqual(sheet["A1"].value, "Demographics")
        self.assertEqual(sheet["A1"].fill.fgColor.rgb, "00CC0000")
        self.assertEqual(sheet["A1"].font.color.rgb, "00FFFFFF")

    def test_question_block_includes_filter_ui_rows(self) -> None:
        _path, workbook, _log = _build_workbook()
        sheet = _data_sheet(workbook)
        self.assertEqual(sheet["A3"].value, "Q1 - Choice")
        self.assertEqual(
            [sheet.cell(row=4, column=col).value for col in range(1, 6)],
            ["Per-question filter", "Filter Q", "(None)", "Value", "(All)"],
        )
        self.assertEqual(sheet["F4"].value, '=IFERROR(INDEX(All_Questions_Data_Names,MATCH(Q1_Q,All_Questions,0)),"\'_RawData\'!$A:$A")')
        self.assertEqual(sheet["G4"].value, '="|" & SUBSTITUTE(Q1_V, ", ", "|") & "|"')
        self.assertEqual(sheet["A5"].value, "Cross-tab by")
        self.assertEqual(sheet["C5"].value, "(None)")
        self.assertTrue(str(sheet["A6"].value).startswith("Note: This question was shown"))
        self.assertIsNone(sheet["A7"].value)
        self.assertEqual(sheet["A8"].value, "Option ID")

    def test_data_body_cells_use_arial(self) -> None:
        _path, workbook, _log = _build_workbook()
        sheet = _data_sheet(workbook)
        self.assertEqual(sheet["C9"].font.name, "Arial")
        self.assertEqual(sheet["D9"].font.name, "Arial")
        self.assertEqual(sheet["G9"].font.name, "Arial")

    def test_single_select_number_formats_are_applied(self) -> None:
        _path, workbook, _log = _build_workbook()
        sheet = _data_sheet(workbook)
        self.assertEqual(sheet["C9"].number_format, "#,##0")
        self.assertEqual(sheet["D9"].number_format, "0.0%")
        self.assertEqual(sheet["E9"].number_format, "#,##0")
        self.assertEqual(sheet["F9"].number_format, "0.0%")
        self.assertEqual(sheet["G9"].number_format, "+0.0%;-0.0%")

    def test_title_bar_has_bain_red_fill(self) -> None:
        _path, workbook, _log = _build_workbook()
        sheet = _data_sheet(workbook)
        self.assertEqual(sheet["A3"].fill.fgColor.rgb, "FFCC0000")
        self.assertEqual(sheet["A3"].font.color.rgb, "FFFFFFFF")
