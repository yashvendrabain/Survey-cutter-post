...
class TestExcelExporter(unittest.TestCase):
    ...
    def test_raw_data_numeric_metric_columns_are_numbers_when_mixed_with_na_tokens(self) -> None:
        dataframe = pd.DataFrame(
            {
                "respondent_id": [1, 2, 3, 4],
                "Q_GRID_RATEDr1": ["4", "5", "N/A", "3"],
                "Q_DIRECT_NUM": ["10.5", "N/A", "", 7],
                "Q_CATEGORY": ["4", "5", "N/A", "3"],
            }
        )
        grid_result = GridRatedResult(
            question_id="Q_GRID_RATED",
            question_type=QuestionType.GRID_RATED,
            valid_n=3,
            missing_n=1,
            denominator_policy=DenominatorPolicy.VALID_RESPONSES,
            question_text="Rate importance",
            column_headers=["All"],
            rows=[
                GridRatedRow(
                    row_id="Q_GRID_RATEDr1",
                    row_label="Reduces time spent on key workflows",
                    means_per_column=[4.0],
                    valid_n_per_column=[3],
                )
            ],
            total_respondents=3,
            total_responses=3,
        ...
        numeric_result = NumericResult(
            ...
            min_val=7.0,
            max_val=10.5,
            percentiles={25: 7.875, 50: 8.75, 75: 9.625},
        )
        category_result = SingleSelectResult(
            ...
            denominator_policy=DenominatorPolicy.VALID_RESPONSES,
            distribution={
                "4": {"label": "4", "count": 1, "rate": 0.25},
                "5": {"label": "5", "count": 1, "rate": 0.25},
                "N/A": {"label": "N/A", "count": 1, "rate": 0.25},
                "3": {"label": "3", "count": 1, "rate": 0.25},
            },
        )
        schema = SurveySchema(
            questions=(
                QuestionSpec(
                    ...
                    question_text="Rate importance",
                    question_type=QuestionType.GRID_RATED,
                    raw_columns=("Q_GRID_RATEDr1",),
                    option_map={index: str(index) for index in range(1, 6)},
                    value_range=(1, 5),
                    grid_row_labels={
                        "Q_GRID_RATEDr1": "Reduces time spent on key workflows",
                    },
                    possible_role="GRID_RATED",
                ),
                QuestionSpec(
                    question_id="[Q_DIRECT_NUM]",
                    canonical_id="Q_DIRECT_NUM",
                    question_text="Direct numeric",
                    question_type=QuestionType.DIRECT_NUMERIC,
                    raw_columns=("Q_DIRECT_NUM",),
                    option_map={},
                ),
                QuestionSpec(
                    question_id="[Q_CATEGORY]",
                    canonical_id="Q_CATEGORY",
                    question_text="Categorical numeric-looking values",
                    question_type=QuestionType.SINGLE_SELECT,
                    raw_columns=("Q_CATEGORY",),
                    option_map={"4": "4", "5": "5", "N/A": "N/A", "3": "3"},
                ),
            ),
            respondent_id_column="respondent_id",
            total_respondents=4,
        ...
        output_path = self.export_custom_workbook(
            [grid_result, numeric_result, category_result],
            schema,
            decoded_df=dataframe,
        )
        workbook = load_workbook(output_path, read_only=False, data_only=False)
        ...
        self.assertEqual(
            [raw_ws.cell(row=row, column=headers["Q_GRID_RATEDr1"]).value for row in range(2, 6)],
            [4, 5, None, 3],
        )
        self.assertEqual(
            [raw_ws.cell(row=row, column=headers["Q_DIRECT_NUM"]).value for row in range(2, 6)],
            [10.5, None, None, 7],
        )
        self.assertEqual(
            [raw_ws.cell(row=row, column=headers["Q_CATEGORY"]).value for row in range(2, 6)],
            ["4", "5", "N/A", "3"],
        )
        self.assertEqual(
            [raw_ws.cell(row=row, column=headers["Q_GRID_RATEDr1"]).data_type for row in (2, 3, 5)],
            ["n", "n", "n"],
        )
        self.assertEqual(raw_ws.cell(row=4, column=headers["Q_GRID_RATEDr1"]).value, None)
        self.assertEqual(raw_ws.cell(row=4, column=headers["Q_CATEGORY"]).data_type, "s")
    ...
    def test_cross_tab_table_has_2d_count_only_layout(self) -> None:
        ...
        self.addCleanup(workbook.close)
        ws = workbook["All Questions"]
        single_cut_header_row = table_header_row(ws, "Q_SS_EXPORT")
        single_cut_total_row = next(
            row
            for row in range(single_cut_header_row + 1, ws.max_row + 1)
            if ws.cell(row=row, column=1).value == "Total respondents"
        )
        header_row = next(
            row
            for row in range(single_cut_total_row + 1, ws.max_row + 1)
            if ws.cell(row=row, column=1).value == "Option"
            and "Q_SS_EXPORT_CT" in str(ws.cell(row=row, column=2).value)
        )
        cross_tab_first_col = 1
        max_groups = 12
    ...
    def test_grid_single_select_excel_format(self) -> None:
        ...
        header_row = next(
            row
            for row in range(question_header_row(ws, "Q_GRID_FORMAT"), ws.max_row + 1)
            if [ws.cell(row, col).value for col in range(1, 5)]
            == ["Option", "Count", "%", "Denominator"]
        )
        self.assertEqual(
            [ws.cell(header_row, col).value for col in range(1, 5)],
    ...
    def test_grid_rated_parent_does_not_render_cross_tab_by_control_or_matrix(self) -> None:
        result, schema = rated_grid_export_fixture()
        output_path = self.export_custom_workbook([result], schema)
        workbook = load_workbook(output_path, read_only=False, data_only=False)
        self.addCleanup(workbook.close)
        ...
        self.assertNotIn("Cross-tab by", values)
        self.assertEqual(formulas, [])
        self.assertNotIn("All_Questions_Q_RATED_CT", workbook.defined_names)
    ...
    def test_rank_cross_tab_by_has_selectable_rank_metric_formula(self) -> None:
        ...
        self.assertIn("All_Questions_Q_RANK_PTS", workbook.defined_names)
        self.assertIn("All_Questions_Q_RANK_RP", workbook.defined_names)
        self.assertEqual(
            defined_range_values(workbook, "All_Questions_Q_RANK_PTS"),
            [10.0, 6.0, 3.0, 0.0],
        )
        rank_metric_cells = [
        ...
        subheaders = [
            ...
            and "Weighted Average" in cell.value
        ]
        ...
        self.assertTrue(
            any(
                ...
                and "INDEX(All_Questions_Q_RANK_PTS,1)*COUNTIFS(Q_RANK_A_data,1" in formula
        ...
        self.assertFalse(any("*100" in formula for formula in formulas), formulas)
    ...
    def test_grid_binary_pivot_does_not_render_cross_tab_by_control_or_matrix(self) -> None:
        ...
        title_row = next(
            row
            for row in range(1, ws.max_row + 1)
            if isinstance(ws.cell(row=row, column=1).value, str)
            and ws.cell(row=row, column=1).value.startswith("Q_GB -")
        )
        values = [
            ws.cell(row=row, column=1).value
            for row in range(title_row, ws.max_row + 1)
        ...
        self.assertNotIn("Cross-tab by", values)
        self.assertEqual(formulas, [])
        self.assertNotIn("All_Questions_Q_GB_CT", workbook.defined_names)
    ...
    def test_grid_categorical_parent_renders_single_count_matrix(self) -> None:
        ...
        formulas = [
            ...
            for row in ws.iter_rows()
            for cell in row
            if isinstance(cell.value, str) and "Q_CATGRID_CT" in cell.value
        ]
        self.assertNotIn("Cross-tab by", values)
        self.assertEqual(formulas, [])
        self.assertNotIn("All_Questions_Q_CATGRID_CT", workbook.defined_names)
    ...
    def test_export_writes_cross_tab_body(self) -> None:
        ...
        cross_tab_header_row = next(
            row
            for row in range(total_row + 1, ws.max_row + 1)
            if ws.cell(row=row, column=1).value == "Option"
            and "Cross_Tab_Questions_Options_Names" in str(ws.cell(row=row, column=2).value)
        )

        self.assertEqual(cross_tab_header_row, total_row + 2)
        self.assertIsNone(ws.cell(row=total_row + 1, column=1).value)
        self.assertEqual(ws.cell(row=cross_tab_header_row, column=1).value, "Option")
        self.assertIn("INDEX(INDIRECT", ws.cell(row=cross_tab_header_row, column=2).value)
        self.assertIn(
            "Cross_Tab_Questions_Options_Names",
            ws.cell(row=cross_tab_header_row, column=2).value,
        )
    ...
    def test_cross_tab_sheet_includes_axis_labels(self) -> None:
        ...
        self.assertEqual(ws.cell(cross_tab_header_row, 1).value, "Option")
        self.assertIn("Q_SS_EXPORT_CT", ws.cell(cross_tab_header_row, 2).value)
    ...
    def test_cross_tab_sheet_has_corner_orientation_label(self) -> None:
        ...
        self.assertEqual(ws.cell(cross_tab_header_row, 1).value, "Option")
        self.assertEqual(ws.cell(cross_tab_header_row + 2, 1).value, "Yes")
