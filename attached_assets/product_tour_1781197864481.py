...
(function(){
  ...
  if (__FORCE__){
    // Retry several times in case the parent DOM/body isn't ready yet — this is
    // the robustness fix for "take the tour does nothing".
    var tries = 0;
    var iv = setInterval(function(){
      tries++;
      if (doc && doc.body){ win.__saeBuild(); }
      if (tries > 12 || doc.getElementById("sae-tour-card")){ clearInterval(iv); }
    }, 120);
    return;
  }
...
def maybe_render_tour(force: bool = False) -> None:
    ...
    # Height must be > 0 for Streamlit to reliably execute the injected <script>.
    # A 1px iframe frequently gets skipped; 10px is safe and visually negligible
    # (the actual tour UI is injected into the PARENT document, not this iframe).
    components_html(_tour_html(TOUR_STEPS, force=forced), height=10)
