...
def _question_label(schema: Any, question_id: str) -> str:
    ...
    if question is None:
        return question_id
    return f"{question.canonical_id}: {_truncate(question.question_text, 90)}"
...
def _render_outcome_summary_panel(seg: OutcomeSegmentationResult) -> None:
    ...
    for rank, diff in enumerate(seg.differentiators[:10], start=1):
        with app.container():
            col1, col2, col3, col4 = app.columns([3, 1, 1, 1])
            col1.markdown(f"**{rank}. {_truncate(diff.question_text, 70)}**")
            col2.caption("Cramér's V")
            col2.markdown(f"V={diff.cramers_v:.3f}")
            col3.caption("Winner Rate")
            col3.markdown(f"{diff.top_option_winner_rate:.1%}")
            col4.caption("Lift")
            lift_text = "∞" if diff.top_option_lift >= 900 else f"{diff.top_option_lift:.2f}x"
            col4.markdown(lift_text)
...
def _render_winner_profile_panel(seg: OutcomeSegmentationResult) -> None:
    ...
    for trait in profile.defining_traits:
        with app.container():
            ...
            col1.caption(_truncate(trait.question_text, 80))
