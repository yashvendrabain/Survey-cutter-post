    ...
    return [
        {
            ...
            "Title": _cross_cut_display_title(result, schema),
            "Type": result.analysis_type.value,
            "Source Questions": _source_question_labels(result.source_question_ids, schema),
...
def _cross_cut_display_title(result: Any, schema: Any) -> str:
    ...
    return _source_question_labels(source_question_ids, schema, include_ids=False, max_chars=50)
...
def _render_outcome_summary_panel(seg: OutcomeSegmentationResult) -> None:
    ...
    table_rows = [
        {
            "#": rank,
            "Question": _truncate(diff.question_text, 60),
            "Top Option": diff.top_option_label,
            "Cramér's V": f"{diff.cramers_v:.3f}",
