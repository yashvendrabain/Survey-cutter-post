...
NUMERIC_LABEL_RE = re.compile(r"^\s*(-?\d+(?:\.\d+)?)")
...
def parse_datamap(path: str) -> DataMap:
    ...
    try:
        ...
        datamap_format = _detect_datamap_format(worksheet)
        if datamap_format == "compact_two_column":
        ...
        if datamap_format == "unknown":
            raise ValueError(
                "Unrecognized data map format. Currently supported: "
                "BCN-style multi-column, two-column compact."
            )
...
def derive_numeric_label_metadata(
    ...
    na_labels: set[str] = set()
    for label in labels:
        match = NUMERIC_LABEL_RE.match(label)
        if match:
            label_to_numeric_value[label] = float(match.group(1))
        else:
            na_labels.add(label)
