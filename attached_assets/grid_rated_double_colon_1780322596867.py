...
def apply_grid_rated_double_colon_matching(
    ...
    promoted["sub_columns"] = [
        (raw_column, _cell_label(dim1_label, dim2_label))
...
def _parse_double_colon_cells(
    ...
    for raw_column in raw_columns:
        ...
        if not dim2_label:
            ...
            continue
        if _is_helper_label(dim1_label):
            continue
...
def _grid_structure_warnings(
    ...
    if _has_blank_dim1_only(cells) and len(dim2_values) >= 2:
        return warnings
...
def _cell_label(dim1_label: str, dim2_label: str) -> str:
    if not dim1_label.strip():
        return dim2_label
    return f"{dim1_label} :: {dim2_label}"


def _has_blank_dim1_only(cells: list[tuple[str, str, str]]) -> bool:
    return bool(cells) and all(not dim1_label.strip() for _raw, dim1_label, _dim2 in cells)


def _is_helper_label(label: str) -> bool:
    normalized = str(label).replace("\u2019", "'").strip().casefold()
    return normalized in {"i don't know", "don't know", "n/a", "not applicable", "no answer"}
