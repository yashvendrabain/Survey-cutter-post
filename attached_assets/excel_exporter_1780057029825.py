...
def _wvl_write_question_block(
    ...
    if isinstance(result, SingleSelectResult):
        return _wvl_write_single_select_layout(worksheet, table_start_row, result, question, context, log, winner_label, laggard_label, fq_name, theme_prefix)
    if isinstance(result, MultiSelectResult):
        return _wvl_write_multi_select_layout(worksheet, table_start_row, result, question, context, log, winner_label, laggard_label, fq_name, theme_prefix)
...
def _wvl_write_single_select_layout(
    ...
    headers = [
        "Option ID",
        "Option",
        f"{winner_label} count",
        f"{winner_label} %",
        f"{laggard_label} count",
        f"{laggard_label} %",
        "Others count",
        "Others %",
        "Total count",
    ...
    for option_code, payload in result.distribution.items():
        ...
        others_count_cell = _live_formula(worksheet, row_index, 7, others_count_formula, 0)
        others_count_cell.number_format = "#,##0"
        _record_wvl_metric(log, worksheet.title, result.question_id, (data_name,), "others_count", others_count_formula, 0)
        others_pct_formula = f'=IFERROR(G{row_index}/$C${others_total_row},0)'
        others_pct_cell = _live_formula(worksheet, row_index, 8, others_pct_formula, 0)
        others_pct_cell.number_format = "0.0%"
        _record_wvl_metric(log, worksheet.title, result.question_id, (data_name,), "others_pct", others_pct_formula, 0)
        total_count_cell = _live_formula(worksheet, row_index, 9, total_count_formula, 0)
        total_count_cell.number_format = "#,##0"
        _record_wvl_metric(log, worksheet.title, result.question_id, (data_name,), "total_count", total_count_formula, 0)
        total_pct_formula = f'=IFERROR(I{row_index}/$C${total_respondents_row},0)'
        total_pct_cell = _live_formula(worksheet, row_index, 10, total_pct_formula, 0)
        total_pct_cell.number_format = "0.0%"
        _record_wvl_metric(log, worksheet.title, result.question_id, (data_name,), "total_pct", total_pct_formula, 0)
...
def _wvl_write_multi_select_layout(
    ...
    for option_id, payload in result.selections.items():
        ...
        winner_count_cell = _live_formula(worksheet, row_index, 3, _wvl_countifs(column.data_name, "Selected", fq_name, "winners_mask_data", theme_prefix), 0)
        winner_count_cell.number_format = "#,##0"
        winner_pct_cell = _live_formula(worksheet, row_index, 4, f'=IFERROR(C{row_index}/$C${winner_total_row},0)', 0)
        winner_pct_cell.number_format = "0.0%"
        laggard_count_cell = _live_formula(worksheet, row_index, 5, _wvl_countifs(column.data_name, "Selected", fq_name, "laggards_mask_data", theme_prefix), 0)
        laggard_count_cell.number_format = "#,##0"
        laggard_pct_cell = _live_formula(worksheet, row_index, 6, f'=IFERROR(E{row_index}/$C${laggard_total_row},0)', 0)
        laggard_pct_cell.number_format = "0.0%"
        others_count_cell = _live_formula(worksheet, row_index, 7, _wvl_countifs(column.data_name, "Selected", fq_name, "others_mask_data", theme_prefix), 0)
        others_count_cell.number_format = "#,##0"
        others_pct_cell = _live_formula(worksheet, row_index, 8, f'=IFERROR(G{row_index}/$C${others_total_row},0)', 0)
        others_pct_cell.number_format = "0.0%"
        total_count_cell = _live_formula(worksheet, row_index, 9, _wvl_total_countifs(column.data_name, "Selected", fq_name, theme_prefix), 0)
        total_count_cell.number_format = "#,##0"
        total_pct_cell = _live_formula(worksheet, row_index, 10, f'=IFERROR(I{row_index}/$C${total_respondents_row},0)', 0)
        total_pct_cell.number_format = "0.0%"
...
def _wvl_write_totals(
    ...
    winner_total_cell = _live_formula(worksheet, winner_total_row, 3, _wvl_countifs(data_name, "<>", fq_name, "winners_mask_data", theme_prefix), cache_value)
    winner_total_cell.number_format = "#,##0"
    worksheet.cell(row=laggard_total_row, column=1, value="Laggards total")
    laggard_total_cell = _live_formula(worksheet, laggard_total_row, 3, _wvl_countifs(data_name, "<>", fq_name, "laggards_mask_data", theme_prefix), cache_value)
    laggard_total_cell.number_format = "#,##0"
    worksheet.cell(row=others_total_row, column=1, value="Others total")
    others_total_cell = _live_formula(worksheet, others_total_row, 3, _wvl_countifs(data_name, "<>", fq_name, "others_mask_data", theme_prefix), cache_value)
    others_total_cell.number_format = "#,##0"
    worksheet.cell(row=total_respondents_row, column=1, value="Total respondents")
    total_cell = _live_formula(worksheet, total_respondents_row, 3, _wvl_total_respondents_formula(fq_name, theme_prefix), cache_value)
