...
def _render_wizard_top_nav() -> None:
    ...
    # ---- Visual stepper (single HTML block, perfectly aligned) ----
    def _circle(n: int) -> str:
        if n < step:
            ...
            bg = "#F2F3F5"; col = "#A8ADB3"; bd = "1.5px solid #E2E4E8"; mark = str(n)
        lab_color = "#CC0000" if n == step else "#999"
        lab_weight = "700" if n == step else "500"
        return (
            "<div style='display:flex;flex-direction:column;align-items:center;flex:0 0 auto;'>"
            f"<div style='width:36px;height:36px;border-radius:50%;background:{bg};color:{col};"
            f"border:{bd};display:flex;align-items:center;justify-content:center;font-weight:700;"
            f"font-size:15px;box-shadow:0 2px 6px rgba(204,0,0,0.18);'>{mark}</div>"
            f"<div style='margin-top:6px;font-size:11px;font-weight:{lab_weight};color:{lab_color};"
            f"font-family:Arial,sans-serif;white-space:nowrap;'>{label_html}</div>"
        ).replace("{label_html}", html.escape(labels[n - 1]))

    def _conn(after_step: int) -> str:
        done = after_step < step
        return (
            "<div style='flex:1 1 60px;height:2px;margin:0 8px;align-self:flex-start;"
            "margin-top:17px;background:"
            f"{'#CC0000' if done else '#E2E4E8'};'></div>"
        )

    app.markdown(
        "<div style='display:flex;align-items:flex-start;justify-content:center;"
        "max-width:560px;margin:4px auto 6px;'>"
        + _circle(1) + _conn(1) + _circle(2) + _conn(2) + _circle(3)
        + "</div>",
        unsafe_allow_html=True,
    )

    # ---- Clickable controls beneath: Back · 1 2 3 · Next (tight, centered) ----
    app.markdown(
        "<style>.wizstep-ctrls + div [data-testid='stHorizontalBlock']{"
        "max-width:360px;margin:0 auto;gap:4px !important;}"
        ".wizstep-ctrls + div .stButton button{padding:2px 0 !important;font-size:11px !important;"
        "border-radius:6px !important;}</style><div class='wizstep-ctrls'></div>",
        unsafe_allow_html=True,
    )
    bcol, s1, s2, s3, ncol = app.columns([1.4, 1, 1, 1, 1.4])
    if bcol.button("\u2039 Back", disabled=(step <= 1), key="wiz_back_top", use_container_width=True):
        app.session_state["wiz_step"] = max(1, step - 1)
        _wizard_rerun(app)
    for idx, scol in enumerate([s1, s2, s3], start=1):
        if scol.button(str(idx), key=f"wiztop_circle_{idx}",
                       type=("primary" if idx == step else "secondary"),
                       use_container_width=True):
            app.session_state["wiz_step"] = idx
            _wizard_rerun(app)
    if ncol.button("Next \u203a", disabled=(step >= 3), key="wiz_next_top",
