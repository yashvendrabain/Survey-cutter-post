...
def _render_nav_bar() -> None:
    ...
    primary = [
        ("section-upload",     "Upload survey",   "\u2713" if has_data else None, "check"),
        ("section-singlecuts", "View results",    str(n_singlecuts) if n_singlecuts else None, None),
        ("section-downloads",  "Download workbook", None, None),
    ]
    more = [
        ("section-filter",    "Apply filters",       str(n_filters) if n_filters else None, None),
        ("section-crosscuts", "Build a cross cut",   str(n_crosscuts) if n_crosscuts else None, None),
        ("section-ai",        "Winners vs Laggards", str(n_diffs) if n_diffs else None, None),
    ]
