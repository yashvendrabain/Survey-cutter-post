...
SESSION_DEFAULTS = {
    ...
    "wiz_step": 1,
    "wiz_prepared": False,
    "wiz_upload_signature": None,
    "wiz_auto_themes": None,
    "wiz_demo_priority": None,
    "wiz_short_labels": {},
    "wiz_category_assignments": {},
    "wiz_selected_demographics": [],
    "wiz_num_custom_filters": 2,
    "wiz_num_per_question_filters": 1,
    "wiz_crosscut_consolidation": "one_sheet",
    "wiz_pending_remove": None,
}
...
def _rerun() -> None:
    ...
    if hasattr(app, "rerun"):
        app.rerun()
    elif hasattr(app, "experimental_rerun"):
        app.experimental_rerun()
...
def _prepare_pipeline(raw_data_path: str, datamap_path: str, status: Any) -> None:
    ...
    status.update(label="Preparing workbook setup...", state="running")
    questions_for_ai = [
        {
    ...
    demo_questions = [
        ...
        for question in schema.demographic_questions()
    ]
    ...
    short_labels = generate_short_labels(
        [
            {"question_id": question.canonical_id, "question_text": question.question_text}
            for question in schema.questions
        ],
        cache=_INSIGHT_CACHE,
    )
    ...
    app.session_state["skips"] = []
    app.session_state["log"] = None
    app.session_state["output_path"] = None
    ...
    app.session_state["wiz_prepared"] = True
    app.session_state["wiz_step"] = 1
    app.session_state["wiz_auto_themes"] = auto_themes
    app.session_state["wiz_demo_priority"] = demo_priority
    app.session_state["wiz_short_labels"] = short_labels
    app.session_state["wiz_category_assignments"] = category_assignments_from_themes(
        schema,
        auto_themes,
    )
    app.session_state["wiz_selected_demographics"] = selected_demographics_from_schema(schema)
    app.session_state["wiz_num_custom_filters"] = normalise_custom_filter_count(
        app.session_state.get("wiz_num_custom_filters")
    )
    app.session_state["wiz_num_per_question_filters"] = normalise_per_question_filter_count(
        app.session_state.get("wiz_num_per_question_filters")
    )
    app.session_state["wiz_crosscut_consolidation"] = app.session_state.get(
        "wiz_crosscut_consolidation",
        "one_sheet",
    )
    status.update(label="Setup ready.", state="complete")


def _run_pipeline(raw_data_path: str, datamap_path: str, status: Any) -> None:
    _prepare_pipeline(raw_data_path, datamap_path, status)
    _run_analysis_from_wizard(status)


def _run_analysis_from_wizard(status: Any) -> None:
    ...
    if schema is None or dataframe is None or quality_report is None:
        raise RuntimeError("Upload and setup must complete before running analysis.")

    category_assignments = dict(app.session_state.get("wiz_category_assignments", {}))
    selected_demographics = list(app.session_state.get("wiz_selected_demographics", []))
    analysis_schema = apply_wizard_schema_overrides(
        schema,
        category_assignments,
        selected_demographics,
    )
    themes = themes_from_category_assignments(analysis_schema, category_assignments)

    status.update(label="Computing single cuts...", state="running")
    log = CalculationLog()
    results, skips = compute_single_cuts(analysis_schema, dataframe, log)

    status.update(label="Exporting workbook...", state="running")
    output_path = str(Path(tempfile.gettempdir()) / "survey_analysis.xlsx")
    export_single_cuts(
        results=results,
        skips=skips,
        ...
        demo_priority=app.session_state.get("wiz_demo_priority"),
        short_labels=app.session_state.get("wiz_short_labels") or None,
    )

    app.session_state["results"] = results
    app.session_state["skips"] = skips
    app.session_state["schema"] = analysis_schema
...
def _refresh_full_workbook() -> None:
    ...
    export_single_cuts(
        ...
        themes=themes_from_category_assignments(
            ...
            app.session_state.get("wiz_category_assignments", {}),
        ),
        decoded_df=app.session_state.get("dataframe"),
        demo_priority=app.session_state.get("wiz_demo_priority"),
        short_labels=app.session_state.get("wiz_short_labels") or None,
    )
...
def _upload_signature(raw_upload: Any | None, datamap_upload: Any | None) -> tuple | None:
    if raw_upload is None or datamap_upload is None:
        return None
    return (
        getattr(raw_upload, "name", ""),
        getattr(raw_upload, "size", 0),
        getattr(datamap_upload, "name", ""),
        getattr(datamap_upload, "size", 0),
    )


def _handle_upload_preparation(raw_upload: Any | None, datamap_upload: Any | None) -> None:
    app = _require_streamlit()
    signature = _upload_signature(raw_upload, datamap_upload)
    if signature is None:
        app.info("Upload both files to start workbook setup.")
        return
    if app.session_state.get("wiz_prepared") and app.session_state.get("wiz_upload_signature") == signature:
        return

    raw_data_path = None
    datamap_path = None
    app.session_state["run_complete"] = False
    app.session_state["wiz_prepared"] = False
    app.session_state["wiz_upload_signature"] = signature

    try:
        raw_data_path = _write_upload_to_temp(raw_upload)
        datamap_path = _write_upload_to_temp(datamap_upload)
        with app.status("Preparing setup wizard...", expanded=True) as status:
            _prepare_pipeline(raw_data_path, datamap_path, status)
    except Exception as exc:  # noqa: BLE001 - UI must show preparation failures.
        app.session_state["wiz_prepared"] = False
        app.session_state["run_complete"] = False
...
def _render_wizard_stepper() -> None:
    app = _require_streamlit()
    current_step = int(app.session_state.get("wiz_step", 1))
    columns = app.columns(len(WIZARD_STEPS))
    for column, (step_number, label) in zip(columns, WIZARD_STEPS):
        state = "done" if step_number < current_step else "current" if step_number == current_step else ""
        circle = "&#10003;" if step_number < current_step else str(step_number)
        with column:
            app.markdown(
                f"""
                <div style="text-align:center;">
                    <div class="wizard-step-circle {state}">{circle}</div>
                    <div class="wizard-step-label {state}">{html.escape(label)}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
def _render_setup_wizard() -> None:
    app = _require_streamlit()
    if not app.session_state.get("wiz_prepared"):
        return
    ...
    app.markdown(
        ...
        <div class="wizard-shell">
            <div class="wizard-kicker">Workbook setup</div>
            <div class="wizard-title">Configure analysis output</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    _render_wizard_stepper()
    app.markdown('<div class="wizard-divider"></div>', unsafe_allow_html=True)
    ...
    if step == 1:
        ...
        _render_wizard_step_cross_cut()

    app.markdown('<div class="wizard-divider"></div>', unsafe_allow_html=True)
    _render_wizard_navigation()


def _wizard_category_names() -> list[str]:
    assignments = _require_streamlit().session_state.get("wiz_category_assignments", {})
    names = sorted({str(value) for value in assignments.values() if str(value).strip()})
    return names or ["All Questions"]


def _rename_wizard_category(old_name: str, new_name: str) -> None:
    app = _require_streamlit()
    new_name = new_name.strip()
    if not new_name or new_name == old_name:
        return
    assignments = dict(app.session_state.get("wiz_category_assignments", {}))
    for question_id, category in list(assignments.items()):
        if category == old_name:
            assignments[question_id] = new_name
    app.session_state["wiz_category_assignments"] = assignments


def _render_remove_confirmation() -> None:
    app = _require_streamlit()
    pending_id = app.session_state.get("wiz_pending_remove")
    if not pending_id:
        return
    schema = app.session_state.get("schema")
    lookup = question_lookup(schema) if schema is not None else {}
    question = lookup.get(pending_id)
    label = question.question_text if question is not None else pending_id
    app.warning(f"Remove {label} from the analysis? It will not appear in the output workbook.")
    confirm_col, cancel_col, _ = app.columns([1, 1, 4])
    if confirm_col.button("Confirm remove", key="wiz_confirm_remove", type="primary"):
        assignments = dict(app.session_state.get("wiz_category_assignments", {}))
        assignments.pop(pending_id, None)
        app.session_state["wiz_category_assignments"] = assignments
        app.session_state["wiz_pending_remove"] = None
        _rerun()
    if cancel_col.button("Cancel", key="wiz_cancel_remove"):
        app.session_state["wiz_pending_remove"] = None
        _rerun()
...
def _render_wizard_step_categories() -> None:
    ...
    schema = app.session_state["schema"]
    assignments = dict(app.session_state.get("wiz_category_assignments", {}))
    lookup = question_lookup(schema)

    app.markdown("### Review & edit categories")
    app.markdown(
        '<div class="wizard-note">Review the automatically grouped workbook themes. '
        "Use the controls on each card to move a question, rename a column, or remove a question.</div>",
        unsafe_allow_html=True,
    )
    _render_remove_confirmation()

    add_col, spacer_col = app.columns([1, 3])
    with add_col:
        new_category = app.text_input("New category name", key="wiz_new_category_name")
        if app.button("+ Add category", key="wiz_add_category"):
            category = new_category.strip()
            if category and category not in _wizard_category_names():
                app.session_state.setdefault("wiz_empty_categories", [])
                app.session_state["wiz_empty_categories"].append(category)
                _rerun()
    with spacer_col:
        app.caption("Drag-and-drop is not available in this runtime; card move controls provide the same saved category assignment.")

    empty_categories = app.session_state.get("wiz_empty_categories", [])
    category_names = list(dict.fromkeys([*_wizard_category_names(), *empty_categories]))
    columns = app.columns(min(max(len(category_names), 1), 4))
    for index, category in enumerate(category_names):
        column = columns[index % len(columns)]
        with column:
            renamed = app.text_input(
                "Category",
                value=category,
                key=f"wiz_category_name_{index}_{category}",
                label_visibility="collapsed",
            )
            _rename_wizard_category(category, renamed)
            app.markdown(f"**{html.escape(renamed or category)}**")
            question_ids = [
                question_id
                for question_id, assigned_category in assignments.items()
                if assigned_category == category
            ]
            if not question_ids:
                app.caption("No questions assigned")
            for question_id in question_ids:
                ...
                app.markdown(
                    f"""
                    <div class="wizard-card" title="{html.escape(question.question_text)}">
                        <div class="wizard-card-id">{html.escape(question.canonical_id)}</div>
                        <div class="wizard-card-text">{html.escape(question_display_text(question))}</div>
                        <div class="wizard-card-type">{html.escape(question_type_label(question))}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
                move_target = app.selectbox(
                    "Move to",
                    category_names,
                    index=category_names.index(category),
                    key=f"wiz_move_{question_id}",
                    label_visibility="collapsed",
                )
                if move_target != category:
                    assignments[question_id] = move_target
                    app.session_state["wiz_category_assignments"] = assignments
                    _rerun()
                if app.button("Remove", key=f"wiz_remove_{question_id}"):
                    app.session_state["wiz_pending_remove"] = question_id
                    _rerun()
...
def _render_wizard_step_demographics() -> None:
    ...
    lookup = question_lookup(schema)
    selected = set(app.session_state.get("wiz_selected_demographics", []))

    app.markdown("### Confirm demographic filters")
    app.markdown(
        '<div class="wizard-note">These questions will appear as workbook-level filters. '
        "Unchecked questions stay in the analysis, but not as default filter rows.</div>",
        unsafe_allow_html=True,
    )
    ...
    for question_id in sorted(candidates):
        question = lookup.get(question_id)
        if question is None:
            continue
        col_check, col_text, col_preview = app.columns([1, 4, 3])
        ...
        col_text.markdown(f"**{question.canonical_id}**")
        col_text.caption(question_display_text(question, 90))
        col_preview.caption(distinct_value_preview(dataframe, question))

    available = [
        question_id
        for question_id in eligible_filter_question_ids(schema)
        if question_id not in selected
    ]
    labels = {
        question_id: f"{question_id} - {question_display_text(lookup[question_id], 80)}"
        for question_id in available
        if question_id in lookup
    }
    if available:
        add_choice = app.selectbox(
            "+ Add another question as a filter",
            [""] + available,
            format_func=lambda value: "Select a question" if value == "" else labels.get(value, value),
            key="wiz_add_demo_choice",
        )
        if app.button("Add filter", key="wiz_add_demo_button", disabled=(add_choice == "")):
            selected.add(add_choice)
...
def _render_wizard_step_custom_filters() -> None:
    ...
    app.markdown(
        '<div class="wizard-note">Custom filters let partners set arbitrary filters on the entire workbook '
        "in addition to the demographic ones. How many would you like?</div>",
        unsafe_allow_html=True,
    )
    value = app.number_input(
        ...
        min_value=0,
        max_value=5,
        value=normalise_custom_filter_count(app.session_state.get("wiz_num_custom_filters")),
        step=1,
        key="wiz_num_custom_filters_input",
    )
    app.session_state["wiz_num_custom_filters"] = int(value)
    app.info(f"{int(value)} custom filter rows will appear on the Filters sheet.")
...
def _render_wizard_step_per_question_filters() -> None:
    ...
    app.markdown(
        '<div class="wizard-note">Per-question filters let users apply filters that only affect a single '
        "question's table. How many filter rows per question?</div>",
        unsafe_allow_html=True,
    )
    value = app.number_input(
        "Filter rows per question",
        min_value=0,
        max_value=3,
        value=normalise_per_question_filter_count(app.session_state.get("wiz_num_per_question_filters")),
        step=1,
        key="wiz_num_per_question_filters_input",
    )
    app.session_state["wiz_num_per_question_filters"] = int(value)
    app.info(f"{int(value)} filter row will appear under each question block." if int(value) == 1 else f"{int(value)} filter rows will appear under each question block.")


def _render_wizard_step_cross_cut() -> None:
    app = _require_streamlit()
    app.markdown("### Cross-cut output format")
    app.markdown(
        '<div class="wizard-note">When you generate cross-cuts from the dashboard, where should they live '
        "in the workbook?</div>",
        unsafe_allow_html=True,
    )
    options = {
        "One consolidated sheet": "one_sheet",
        "Separate sheet per cross-cut": "separate_sheets",
    }
    current = app.session_state.get("wiz_crosscut_consolidation", "one_sheet")
    current_label = next((label for label, value in options.items() if value == current), "One consolidated sheet")
    selected = app.radio(
        "Cross-cut workbook layout",
        list(options),
        index=list(options).index(current_label),
        captions=[
            'All cross-cuts on a single "Cross_Cuts" sheet',
            "Each cross-cut on its own sheet, for example CC_01 and CC_02",
        ],
    )
    app.session_state["wiz_crosscut_consolidation"] = options[selected]


def _render_wizard_navigation() -> None:
    app = _require_streamlit()
    step = int(app.session_state.get("wiz_step", 1))
    back_col, counter_col, next_col = app.columns([1, 1, 1])
    if back_col.button("Back", disabled=(step <= 1), key="wiz_back"):
        app.session_state["wiz_step"] = max(1, step - 1)
        _rerun()
    counter_col.markdown(
        f"<div style='text-align:center;color:#666666;font-size:14px;'>Step {step} of 5</div>",
        unsafe_allow_html=True,
    )
    if step < 5:
        if next_col.button("Next", type="primary", key="wiz_next"):
            app.session_state["wiz_step"] = min(5, step + 1)
            _rerun()
    else:
        if next_col.button("Run Analysis ->", type="primary", key="wiz_run_analysis"):
            try:
                app.session_state["run_complete"] = False
                with app.status("Running analysis...", expanded=True) as status:
                    _run_analysis_from_wizard(status)
            except Exception as exc:  # noqa: BLE001 - UI must show analysis failures.
                app.session_state["run_complete"] = False
                app.error(f"{type(exc).__name__}: {exc}")
                with app.expander("Show full traceback"):
                    app.code(traceback.format_exc())
...
def _handle_run(raw_upload: Any | None, datamap_upload: Any | None) -> None:
    app = _require_streamlit()
    _handle_upload_preparation(raw_upload, datamap_upload)
    if app.session_state.get("wiz_prepared") and not app.session_state.get("run_complete"):
...
def _grid_payload_distribution(row_result: Any) -> dict[Any, Any]:
    if isinstance(row_result, dict):
        distribution = row_result.get("distribution", {})
    else:
        distribution = getattr(row_result, "distribution", {})
    return distribution if isinstance(distribution, dict) else {}


def _grid_option_count(row_result: Any) -> int:
    count = 0
    for code, payload in _grid_payload_distribution(row_result).items():
        if not isinstance(payload, dict):
            continue
        option_count = int(payload.get("count", 0) or 0)
        option_rate = float(payload.get("rate", 0.0) or 0.0)
        if code not in (0, "0") and (option_count > 0 or option_rate > 0.0):
            count += option_count
    return count


def _build_grid_display_rows(result: Any, schema: Any | None = None) -> list[dict[str, Any]]:
    if isinstance(result, dict):
        grid_rows = result.get("rows", {})
        denominator = int(result.get("valid_n") or result.get("overall_valid_n") or 0)
        question_id = result.get("question_id", "")
    else:
        grid_rows = getattr(result, "rows", {})
        denominator = int(getattr(result, "valid_n", 0) or 0)
        question_id = getattr(result, "question_id", "")

    labels: dict[str, str] = {}
    if schema is not None and question_id:
        question = schema.get_question(question_id)
        if question is not None and question.grid_row_labels:
            labels = dict(question.grid_row_labels)

    rows = []
    for sub_column_id, row_result in grid_rows.items():
        count = _grid_option_count(row_result)
        if count == 0:
            continue
        rows.append(
            {
                "Option": labels.get(sub_column_id, sub_column_id),
                "Count": count,
                "%": f"{(count / denominator * 100) if denominator else 0.0:.1f}%",
...
def _preview_segment_profile(result: Any) -> None:
    ...
    if "rows" in target_result:
        rows = _build_grid_display_rows(target_result)
        if rows:
            app.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
        else:
            app.write("No options selected by any respondent.")
        return
