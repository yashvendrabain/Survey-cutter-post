...
def _build_filters_sheet(
    ...
    for question in demographic_questions[:8]:
        ...
        worksheet.cell(
            row=row_index,
            column=1,
            value=_filter_display_label(question),
        )
        ...
        sheet_filters.append(
            {
                ...
                "kind": column.kind,
                "label": _filter_display_label(question),
...
def _build_per_question_filter_helper_column(
    ...
    for row_index in range(2, last_row + 1):
            _live_formula(
                ...
                _raw_custom_filter_match_formula(
                    row_index=row_index,
                    question_name=f"{q_filter_prefix}_Q",
                    value_name=f"{q_filter_prefix}_V",
                    wrapped_name=f"{q_filter_prefix}_wrapped",
                    resolved_column_name=f"{q_filter_prefix}_resolved_column",
                ),
                1,
            )
...
def _live_write_run_summary(
    ...
    # Fix 4 — Surface the static-values safety-valve notice at the top of
    # the Run_Summary so users know live filters are disabled and why.
    if _is_static_only(workbook):
        notice_row = len(rows) + 2
        limit = getattr(workbook, "sie_raw_limit", "configured limit")
        worksheet.cell(
            row=notice_row,
            column=1,
            value="Memory-safety mode:",
        ).font = _live_font(bold=True, color="CC0000")
        worksheet.cell(
            row=notice_row,
            column=2,
            value=(
                f"Raw data exceeded {limit} rows. _RawData was collapsed and "
                f"all formulas were written as pre-computed static values. "
                f"Counts and percentages are correct, but workbook filters "
                f"and dropdowns will not recalculate."
            ),
        ).font = _live_font(italic=True, color="666666")
...
def _live_write_question_block(
    ...
    _build_per_question_filter_helper_column(workbook, q_filter_prefix, context)
    row_index += 1
...
def _live_write_categorical_table(
    ...
    _write_total_respondents_row(worksheet, total_row, total_count, 4)
...
def _live_write_cross_tab_table(
    ...
    for offset in range(8):
        ...
        _live_formula(
            ...
            start_row,
            col_index,
            f'=IFERROR(INDEX(INDIRECT({_question_options_name_formula(ct_name)}),{offset + 2}),"")',
            "",
        )
    ...
    if rows:
        ...
        _apply_color_scale_range(worksheet, data_start, first_col + 1, data_end, first_col + 8)
        _apply_color_scale_range(worksheet, data_start, first_col + 9, data_end, first_col + 9)
...
def _filter_display_label(question: Any) -> str:
    ...
    text = _strip_question_prefix(str(getattr(question, "question_text", question)))
    lowered = text.lower()
    ...
    if "region" in lowered or "geograph" in lowered:
        return "Region"
    if "company size" in lowered or "employees" in lowered or "headcount" in lowered:
        return "Company Size"
...
def _unique_filter_option_labels(columns: list[_LiveColumnSpec]) -> dict[str, str]:
    labels: dict[str, str] = {}
    used: set[str] = {"(None)", "(Inherit)"}
    for column in columns:
        base = _filter_option_label_for_column(column)
        candidate = base
        suffix = 2
...
def _filter_option_label_for_column(column: _LiveColumnSpec) -> str:
    question = column.question
    if question is None:
...
def _question_options_name_formula(question_name: str) -> str:
    return (
        f'XLOOKUP({question_name},All_Questions,'
        f'All_Questions_Options_Names,"None_options")'
    )


def _question_heading_text(question: Any, short_labels: dict[str, str]) -> str:
    text = short_labels.get(question.canonical_id, question.question_text)
    return _strip_question_prefix(str(text)) or str(text)
