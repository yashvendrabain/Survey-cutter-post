...
def render_chat_panel(faq_path: str = _FAQ_PATH_DEFAULT) -> None:
    ...
    # Native open/close toggle (red, no injected launcher).
    st.markdown(
        "<style>#sae-chat-toggle-anchor + div .stButton button{"
        "background:#CC0000;color:#fff;border:none;font-weight:700;border-radius:10px;}"
        "#sae-chat-toggle-anchor + div .stButton button:hover{background:#B30000;}</style>"
        "<div id='sae-chat-toggle-anchor'></div>",
        unsafe_allow_html=True,
    )
    label = "\u2715  Close assistant" if st.session_state[_OPEN_KEY] else "\U0001F4AC  Have a question? Ask away"
    if st.button(label, key="chat_open_toggle"):
    ...
    with st.container(border=True):
        st.markdown(
            ...
            "<div style='width:34px;height:34px;border-radius:50%;background:#CC0000;color:#fff;"
            "display:flex;align-items:center;justify-content:center;font-weight:700;font-size:16px;'>a</div>"
            "<div><div style='font-size:16px;font-weight:700;color:#0A0A0A;'>Survey Assistant</div>"
            "<div style='font-size:12px;color:#888;'>How can we help you today?</div></div></div>",
            unsafe_allow_html=True,
        )
