...
def compute_multi_select(
    ...
    for sub_column_id in question_spec.raw_columns:
        if sub_column_id not in df.columns:
            ...
            continue
        binary_columns[sub_column_id] = df[sub_column_id]
    ...
    if filter_mask is not None:
        binary_columns = {
            sub_column_id: series[filter_mask]
            for sub_column_id, series in binary_columns.items()
        }
    ...
    result_dict, audit = selection_rate(
        ...
        all_respondents_n=len(df) if denominator_policy == "all_respondents" else None,
