...
#sae-chat-hint { position:fixed; right:86px; bottom:34px; background:#0A0A0A; color:#fff;
  font-family:Arial,sans-serif; font-size:12px; padding:6px 10px; border-radius:6px;
  z-index:1500000; opacity:0.92; }
</style>
<button id="sae-chat-launch" title="Assistant">__LABEL__</button>
<div id="sae-chat-hint">Ask the assistant</div>
<script>
(function(){
  ...
  btn.addEventListener("click", function(){
    // Click the hidden Streamlit toggle button named chat_toggle_signal.
...
def render_chat_panel(faq_path: str = _FAQ_PATH_DEFAULT) -> None:
    ...
    # Hidden toggle the launcher's JS clicks. Kept off-screen via CSS in app.
    if st.button("chat_toggle_signal", key="chat_toggle_signal"):
        st.session_state[_OPEN_KEY] = not st.session_state[_OPEN_KEY]
        st.rerun()

    components_html(_launcher_html(st.session_state[_OPEN_KEY]), height=0)
