...
(function(){
  ...
  // Steps + state live on the PARENT window so every rerun-injected copy shares them.
  ...
  function remove(){ var c=doc.getElementById("sae-tour-card"); if(c && c.parentNode) c.parentNode.removeChild(c); }

  function build(){
    var steps = win.__saeSteps;
    if (win.__saeDone){ remove(); return; }
    var i = win.__saeStep; if(i<0) i=0;
    if (i >= steps.length){ win.__saeDone=true; remove(); return; }
    var step = steps[i];
    remove();
    if (!doc.body) return;
  ...
  win.__saeBuild = build;

  // ONE persistent delegated handler on the parent document. Survives DOM swaps
  // and duplicate script injection (we de-dupe with a flag). This is why clicks
  // no longer "do nothing" after a rerun.
  if (!win.__saeWired){
    win.__saeWired = true;
    doc.addEventListener("click", function(e){
      ...
      var act = t.getAttribute("data-sae");
      if (!act) return;
      var steps = win.__saeSteps || [];
      if (act === "back"){ if(win.__saeStep>0){ win.__saeStep--; build(); } }
      else if (act === "next"){
        win.__saeStarted = true;
        if (win.__saeStep < steps.length-1){ win.__saeStep++; build(); }
        else { win.__saeDone=true; remove(); try{win.sessionStorage.setItem("sae_tour_seen","1");}catch(err){} }
      } else if (act === "skip"){
        win.__saeDone=true; remove(); try{win.sessionStorage.setItem("sae_tour_seen","1");}catch(err){}
      }
    }, true);
    // drag (also delegated / persistent)
  ...
  if (__FORCE__){ setTimeout(build,120); setTimeout(build,450); return; }
  ...
  if(!seen || win.__saeStep>0){ setTimeout(build,300); }
})();
</script>
