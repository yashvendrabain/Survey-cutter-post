...
def _section_downloads() -> None:
    ...
    _red_instruction_box(
        "Step 4 \u2014 Download",
        "Generate and download your workbook. Open it in Excel desktop so the "
        "live filters work.",
    )
...
def _render_sidebar() -> None:
    ...
    with app.sidebar:
        ...
        # Small, unobtrusive "Take the tour" button (red, icon, no emoji).
        app.markdown(
            "<style>div[data-testid='stSidebar'] .stButton button[kind] "
            "{background:#CC0000;color:#fff;border:none;font-size:12px;font-weight:600;"
            "padding:5px 10px;border-radius:7px;}"
            "div[data-testid='stSidebar'] .stButton button[kind]:hover{background:#B30000;}</style>",
            unsafe_allow_html=True,
        )
        _tour_label = (
            "\u25B6  Take the tour"  # triangle 'play' icon, no emoji
        )
        if app.button(_tour_label, key="take_tour_btn_sidebar"):
            restart_tour()
            app.rerun()

        # Chatbot toggle + panel live in the sidebar, right below the tour button.
        render_chat_panel()
...
def main() -> None:
    ...
    # page (except the dedicated Download view, which already shows it inline).
    # Always open (not a dropdown), visually set off as a footer.
    if view != "download":
        app.markdown(
            "<div style='margin-top:32px;border-top:2px solid #CC0000;padding-top:6px;'>"
            "<div style='font-size:12px;font-weight:700;letter-spacing:0.08em;"
            "text-transform:uppercase;color:#CC0000;'>Download workbooks</div></div>",
            unsafe_allow_html=True,
        )
        _section_downloads()
...
def _render_wizard_top_nav() -> None:
    """Compact chevron nav with the 3 step circles (Categories / Filters /
    Cross-cuts) in the middle, between the ‹ › arrows."""
    app = _require_streamlit()
    step = int(app.session_state.get("wiz_step", 1))
    back_col, mid_col, next_col = app.columns([1, 12, 1])
    if back_col.button(
    ...
    with mid_col:
        ...
        # Compact, clickable step circles. Scoped CSS keeps them small.
        app.markdown(
            "<style>.wiztop-row div[data-testid='column'] .stButton button{"
            "width:34px;height:34px;border-radius:50%;font-size:14px;font-weight:700;"
            "padding:0;margin:0 auto;}</style><div class='wiztop-row'></div>",
            unsafe_allow_html=True,
        )
        c1, c2, c3 = app.columns(3)
        for idx, (col, label) in enumerate(zip([c1, c2, c3], labels), start=1):
            with col:
                if app.button(
                    str(idx), key=f"wiztop_circle_{idx}",
                    type=("primary" if idx == step else "secondary"),
                ):
                    app.session_state["wiz_step"] = idx
                    _wizard_rerun(app)
                weight = "700" if idx == step else "400"
                color = "#CC0000" if idx == step else "#999"
                app.markdown(
                    f"<div style='text-align:center;font-size:11px;font-weight:{weight};"
                    f"color:{color};font-family:Arial,sans-serif;margin-top:1px;'>{label}</div>",
                    unsafe_allow_html=True,
                )
    if step < 3:
        if next_col.button(
