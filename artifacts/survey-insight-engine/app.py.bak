...
def _render_wizard_top_nav() -> None:
    ...
    with mid_col:
        ...
        # Sub-step circles styled like the top journey dots, but smaller + subtler.
        app.markdown(
            "<style>"
            ".wiztop-row + div div[data-testid='column'] .stButton button{"
            ...
            # active (primary) = filled red gradient like journey 'done/active' dot
            ".wiztop-row + div div[data-testid='column'] .stButton button[kind='primary']{"
            "background:linear-gradient(180deg,#E00000,#CC0000) !important;color:#fff !important;"
            "border:none !important;box-shadow:0 2px 6px rgba(204,0,0,0.3) !important;}"
            # inactive (secondary) = light ring, muted
            ".wiztop-row + div div[data-testid='column'] .stButton button[kind='secondary']{"
            "background:#F2F3F5 !important;color:#A8ADB3 !important;border:1.5px solid #E2E4E8 !important;"
            "box-shadow:none !important;}"
            "</style><div class='wiztop-row'></div>",
            unsafe_allow_html=True,
        )
        ...
        for idx, (col, label) in enumerate(zip([c1, c2, c3], labels), start=1):
            with col:
                ...
                app.markdown(
                    ...
                    f"color:{color};font-family:Arial,sans-serif;margin-top:2px;'>{label}</div>",
                    unsafe_allow_html=True,
                )
    if step < 3:
        if next_col.button(
