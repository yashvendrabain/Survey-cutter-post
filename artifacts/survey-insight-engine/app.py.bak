...
def _section_upload() -> None:
    ...
    if app.session_state["run_complete"]:
        ...
        # Stepwise guidance: tell the user exactly what to do next.
        app.markdown(
            "<div style='margin:10px 0 4px;padding:12px 16px;background:#F7F8FA;"
            "border:1px solid #E8EAED;border-radius:10px;font-family:Arial,sans-serif;'>"
            "<span style='font-weight:700;color:#CC0000;font-size:13px;'>Next step \u2192</span> "
            "<span style='font-size:13px;color:#333;'>Click any question in the "
            "<b>left sidebar</b> to see its cut. Want to narrow the whole sample first "
            "(e.g. one region)? Use <b>Apply global filter</b> in the top nav.</span></div>",
            unsafe_allow_html=True,
        )
        if app.button("Apply global filter", key="goto_global_filter_from_counts"):
            _set_current_nav_view("filters")
            app.rerun()
...
def _render_segment_definition_ui() -> None:
    ...
    _SEG_MODE_HELP = (
        "How to define the top group (everyone else becomes 'the rest'):\n\n"
        "\u2022 Categorical \u2014 pick the answer(s) that mean success "
        "(e.g. top group = those who chose '20%+' revenue growth).\n\n"
...
def _render_journey_rail() -> None:
    ...
    next_hint = {
        ...
        2: "\u2192 Next: click a question in the sidebar to see its cut, or open Outcome Segmentation.",
...
def main() -> None:
    ...
    # Persistent FULL download panel — pinned at the bottom of EVERY page (except
    # the dedicated Download view, which already shows it inline). Same 4-workbook
    # panel, in an expander so it's always reachable without scrolling the page away.
    if view != "download":
        app.divider()
        with app.expander("\u2b07  Download workbooks (available on every page)", expanded=False):
            _section_downloads()
...
def _render_wizard_step_filters() -> None:
    ...
    app.caption(
        ...
        "in the Excel output, so a reader can slice that sheet on the fly. Pick the "
        "questions you want available as slicers below."
    )
    # Pretty Excel-style sneak peek: a mini spreadsheet showing where filters sit.
    app.markdown(
        "<div style='margin:8px 0 18px;font-family:Calibri,Arial,sans-serif;"
        "border:1px solid #D4D4D4;border-radius:8px;overflow:hidden;max-width:420px;"
        ...
        "APAC <span style='color:#217346;'>\u25BE</span></span></div></div>"
        "<div style='display:flex;border-bottom:1px solid #EEE;font-size:12px;'>"
        "<div style='width:140px;padding:7px 10px;border-right:1px solid #EEE;color:#333;'>Company size</div>"
        "<div style='flex:1;padding:7px 10px;'>"
        "<span style='display:inline-flex;align-items:center;gap:8px;background:#fff;"
        "border:1px solid #BBB;border-radius:4px;padding:2px 8px;color:#333;'>"
        "Enterprise <span style='color:#999;'>\u25BE</span></span></div></div>"
        # custom filter row (highlighted)
        "<div style='display:flex;font-size:12px;background:#FFFCF2;'>"
        "<div style='width:140px;padding:7px 10px;border-right:1px solid #EEE;color:#9A7B00;font-weight:600;'>"
        "Custom \u00b7 Tenure</div>"
        "<div style='flex:1;padding:7px 10px;'>"
        "<span style='display:inline-flex;align-items:center;gap:8px;background:#fff;"
        "border:1px solid #E0B400;border-radius:4px;padding:2px 8px;color:#9A7B00;font-weight:600;'>"
        "5+ years <span style='color:#E0B400;'>\u25BE</span></span></div></div>"
        "</div>",
        unsafe_allow_html=True,
    )
