...
export_single_cuts(results=results, skips=skips, schema=schema, quality_report=qreport, log=log, output_path="outputs/survey_prompt2_6_sample.xlsx", themes=themes, decoded_df=df, short_labels=short_labels)
print(f"[{time.time()-t0:5.1f}s] sample done size={os.path.getsize('outputs/survey_prompt2_6_sample.xlsx')/1024/1024:.2f}MiB")
