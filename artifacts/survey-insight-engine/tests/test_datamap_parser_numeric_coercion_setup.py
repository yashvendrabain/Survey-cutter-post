...
class TestDatamapParserNumericCoercionSetup(unittest.TestCase):
    def test_q30_style_datamap_builds_numeric_label_metadata(self) -> None:
        rows = [
        ...
        data_map = parse_datamap(_write_datamap(rows))
        question = data_map["questions"][0]

        self.assertEqual(len(question["label_to_numeric_value"]), 11)
        self.assertEqual(question["label_to_numeric_value"]["0 (extremely low)"], 0.0)
        self.assertEqual(question["label_to_numeric_value"]["10 (extremely high)"], 10.0)
        self.assertEqual(
            question["na_label_set"],
            frozenset({"This was not something I considered"}),
        )
        self.assertEqual(question["allowed_numeric_range"], (0.0, 10.0))
